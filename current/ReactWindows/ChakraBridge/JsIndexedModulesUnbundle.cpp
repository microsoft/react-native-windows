...
JsModulesUnbundleModule *JsIndexedModulesUnbundle::GetModule(uint32_t index) {
  // The module data is little-endian, this function assumes a little-endian
  // host
  ...
  // Copy the script
  size_t numConverted;
  IfErrnoCleanup(memcpy_s(
      script,
      moduleData.length,
      byteBuffer + baseOffset + moduleData.offset,
      moduleData.length));
  IfErrnoCleanup(mbstowcs_s(
      &numConverted, wcScript, moduleData.length, script, moduleData.length));
  delete[] script;
...
JsErrorCode JsIndexedModulesUnbundle::GetStartupCode(wchar_t **pszScript) {
  ...
  IfFailRet(ChakraHost::LoadByteCode(
      sourcePath, &byteBuffer, &fileHandle, &mapHandle, true));
