...
int NativeJavaScriptExecutor::SetGlobalVariable(
    String ^ variableName,
    String ^ stringifiedText) {
  JsValueRef valueStringified;
  IfFailRet(JsPointerToString(
      stringifiedText->Data(), stringifiedText->Length(), &valueStringified));
...
ChakraStringResult NativeJavaScriptExecutor::GetGlobalVariable(
    String ^ variableName) {
  JsValueRef globalVariable;
  IfFailRetNullPtr(
      this->host.GetGlobalVariable(variableName->Data(), &globalVariable));

  JsValueRef globalVariableJson;
  IfFailRetNullPtr(
      this->host.JsonStringify(globalVariable, &globalVariableJson));
...
int NativeJavaScriptExecutor::SerializeScript(
    String ^ source,
    String ^ serialized) {
...
int NativeJavaScriptExecutor::RunSerializedScript(
    String ^ source,
    String ^ serialized,
    String ^ sourceUri) {
  JsValueRef result;
  IfFailRet(this->host.RunSerializedScript(
      source->Data(), serialized->Data(), sourceUri->Data(), &result));
  return JsNoError;
}

//
// The next three methods do cache JS method references based on the assumption
// no underlying objects change until the bridge disposing.
//

ChakraStringResult NativeJavaScriptExecutor::CallFunctionAndReturnFlushedQueue(
    String ^ moduleName,
    String ^ methodName,
    String ^ args) {
  ...
  IfFailRetNullPtr(JsPointerToString(
      moduleName->Data(), moduleName->Length(), &moduleNameRef));
  IfFailRetNullPtr(JsPointerToString(
      methodName->Data(), methodName->Length(), &methodNameRef));
  ...
  JsValueRef result;
  JsValueRef newArgs[4] = {
      host.globalObject, moduleNameRef, methodNameRef, jsonObj};
...
ChakraStringResult
NativeJavaScriptExecutor::InvokeCallbackAndReturnFlushedQueue(
    int callbackId,
    String ^ args) {
  CheckAndGetMethodRef(
      L"invokeCallbackAndReturnFlushedQueue", invokeCallbackMethod);
...
void NativeJavaScriptExecutor::SetFlushQueueImmediate(
    FlushQueueImmediateHandler ^ handler) {
...
JsErrorCode NativeJavaScriptExecutor::CheckAndGetMethodRef(
