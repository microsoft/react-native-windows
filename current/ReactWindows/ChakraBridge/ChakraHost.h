...
class ChakraHost {
 public:
  ...
  /// <param name="bIsReadOnly">Signals that the mapped file is only for
  /// reading.</param> <returns> JsNoError is no error, else a JsErrorCode with
  /// the appropriate error.
  /// </returns>
  static JsErrorCode LoadByteCode(
  ...
  /// <summary>
  /// Initializes a JSRT session with the context, globals, JSON, and console
  /// logging.
  /// </summary>
  /// <returns>
  /// JsNoError is no error, else a JsErrorCode with the appropriate error.
  ...
  /// <summary>
  /// Runs the script from the file location with the source URI and returns the
  /// <see cref="JsValueRef" /> result.
  /// </summary>
  ...
  /// </summary>
  JsErrorCode SerializeScript(
      const wchar_t *szPath,
      const wchar_t *szSerializedPath);

  /// <summary>
  /// Runs the serialized script from the serialized file location with source
  /// file path, source URI and returns the <see cref="JsValueRef" /> result.
  /// </summary>
  /// <param name="szPath">The location of the script to run.</param>
  /// <param name="szSerializedPath">The location of the serialized script to
  /// run.</param> <param name="szSourceUri">The source URI of the
  /// script.</param> <param name="result">[Out] The result from running the
  /// script.</param> <returns> JsNoError is no error, else a JsErrorCode with
  /// the appropriate error.
  /// </returns>
  JsErrorCode RunSerializedScript(
      const wchar_t *szPath,
      const wchar_t *szSerializedPath,
      const wchar_t *szSourceUri,
      JsValueRef *result);
  ...
  /// <summary>
  /// Calls JSON stringify on the given <see cref="JsValueRef" /> and returns
  /// the <see cref="JsValueRef" /> result.
  /// </summary>
  ...
  /// <summary>
  /// Calls JSON parse on the given <see cref="JsValueRef" /> and returns the
  /// <see cref="JsValueRef" /> result.
  /// </summary>
