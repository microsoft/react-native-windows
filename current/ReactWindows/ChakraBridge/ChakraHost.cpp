...
// any log messages before NativeJavaScriptExecutor::InitializeHost gets
// initialized
extern "C" std::function<void(int, const wchar_t *)> g_loggingCallback =
    [=](int level, const wchar_t *logline) {
#ifdef _DEBUG
...
JsValueRef CALLBACK NativeLoggingCallback(
    JsValueRef callee,
  ...
  if (g_isLoggingEnabled) {
    ...
    if (_aswprintf(
            &szError,
            L"NativeLoggingCallback: Error '%u' while trying to get a string pointer from JsStringToPointer",
            status) < 0) {
...
JsErrorCode ChakraHost::LoadByteCode(
  ...
  *hFile =
      CreateFile2(szPath, dwShareMode, FILE_SHARE_READ, OPEN_EXISTING, nullptr);
  ...
  DWORD flProtect =
      (bIsReadOnly ? PAGE_READONLY : PAGE_READWRITE) | SEC_RESERVE;
  *hMap = CreateFileMapping(
      *hFile, nullptr, flProtect, 0, 0, L"ReactNativeMapping");
  if (*hMap == NULL) {
  ...
  DWORD dwDesiredAccess =
      bIsReadOnly ? FILE_MAP_READ : FILE_MAP_READ | FILE_MAP_WRITE;
...
JsErrorCode ChakraHost::LoadFileContents(
    const wchar_t *szPath,
    wchar_t **pszData) {
  FILE *file;
  *pszData = nullptr;
  ...
  *pszData = (wchar_t *)calloc(lengthBytes + 1, sizeof(wchar_t));
  ...
  if (MultiByteToWideChar(
          CP_UTF8, 0, rawBytes, lengthBytes + 1, *pszData, lengthBytes + 1) ==
      0) {
...
bool CompareLastWrite(const wchar_t *szPath1, const wchar_t *szPath2) {
  ...
  hPath1 = CreateFile2(
      szPath1, GENERIC_READ, FILE_SHARE_READ, OPEN_EXISTING, nullptr);
  if (hPath1 == INVALID_HANDLE_VALUE &&
      GetLastError() == ERROR_FILE_NOT_FOUND) {
    return false;
  }

  hPath2 = CreateFile2(
      szPath2, GENERIC_READ, FILE_SHARE_READ, OPEN_EXISTING, nullptr);
...
bool CALLBACK LoadSourceCallback(
    JsSourceContext sourceContext,
    const wchar_t **scriptBuffer) {
...
JsValueRef CALLBACK NativeRequire(
    JsValueRef callee,
    bool isConstructCall,
    JsValueRef *arguments,
    unsigned short argumentCount,
  ...
  double index;
  IfFailThrow(
      JsNumberToDouble(arguments[1], &index),
      L"Index parameter to nativeRequire must be a number.");
  uint32_t convertedIndex = (uint32_t)index;
  if (convertedIndex <= 0) {
    ThrowException(
        L"Index parameter to nativeRequire must be greater than zero.");
    return JS_INVALID_REFERENCE;
  }
  ...
  if (host->EvaluateScript(module->source, module->sourceUrl, nullptr) !=
      JsNoError) {
...
JsValueRef CALLBACK NativeCallSyncHook(
    JsValueRef callee,
    bool isConstructCall,
    JsValueRef *arguments,
    unsigned short argumentCount,
  ...
  double moduleId;
  IfFailThrow(
      JsNumberToDouble(arguments[1], &moduleId),
      L"The moduleId parameter to nativeCallSyncHook must be a number.");
  double methodId;
  IfFailThrow(
      JsNumberToDouble(arguments[2], &methodId),
      L"The methodId parameter to nativeCallSyncHook must be a number.");

  // Get the stringified arguments
  JsValueRef stringifiedArgs;
  IfFailThrow(
      host->JsonStringify(arguments[3], &stringifiedArgs),
      L"Could not stringify args parameter.");
  const wchar_t *argsBuf;
  size_t bufLen;
  IfFailThrow(
      JsStringToPointer(stringifiedArgs, &argsBuf, &bufLen),
      L"Could not get pointer to stringified args.");

  // Invoke the sync callback
  auto result = host->callSyncHandler(
      (int)moduleId,
      (int)methodId,
      ref new String(argsBuf, (unsigned int)bufLen));
  if (result.ErrorCode == JsNoError) {
    // Return the parsed JSON result
    JsValueRef stringifiedResult;
    IfFailThrow(
        JsPointerToString(
            result.Result->Data(), result.Result->Length(), &stringifiedResult),
        L"Could not convert pointer to stringified result.");
    JsValueRef jsonResult;
    IfFailThrow(
        host->JsonParse(stringifiedResult, &jsonResult),
        L"Could not parse stringified result");
    return jsonResult;
  } else {
    ...
    IfFailThrow(
        JsPointerToString(
            ...
            &stringifiedMessage),
        L"Could not convert pointer to stringified message.");
    JsValueRef jsonError;
    IfFailThrow(
        JsCreateError(stringifiedMessage, &jsonError),
        L"Could not create JavaScript error.");
    IfFailThrow(
        JsSetException(jsonError), L"Could not throw JavaScript error.");
    return JS_INVALID_REFERENCE;
  }
}

JsValueRef CALLBACK NativeFlushQueueImmediate(
    JsValueRef callee,
    bool isConstructCall,
    JsValueRef *arguments,
    unsigned short argumentCount,
  ...
  if (argumentCount != 2) {
    ThrowException(
        L"Expected only one parameter to nativeFlushQueueImmediate.");
    return JS_INVALID_REFERENCE;
  }
  ...
  JsValueRef stringifiedArgs;
  IfFailThrow(
      host->JsonStringify(arguments[1], &stringifiedArgs),
      L"Could not stringify the args parameter.");
  const wchar_t *argsBuf;
  size_t bufLen;
  IfFailThrow(
      JsStringToPointer(stringifiedArgs, &argsBuf, &bufLen),
  ...
  host->flushQueueImmediateHandler(
      ref new String(argsBuf, (unsigned int)bufLen));
...
//
// This function generates a bytecode file ("serialized script") based on the
// original JS bundle. Subsequent applications launches get a speed benefit by
// directly executing this generasted file (with RunSerializedScript)
//
// Some care is taken to make sure there's a way out for any error/corruption
// that may happen. The first version of this code used to write straight to the
// output file. Application being terminated before the operation ended
// triggered the generaion of incomplete files. As a mitigation we use a "write
// to temp file + rename" pattern now.
//
JsErrorCode ChakraHost::SerializeScript(
    const wchar_t *szPath,
    const wchar_t *szSerializedPath) {
  ULONG bufferSize = 0L;
  BYTE *buffer = nullptr;
  ...
  if (!CompareLastWrite(szSerializedPath, szPath)) {
    ...
    size_t tmpFilePathSize =
        wcslen(szSerializedPath) + 4; // add string size of ".tmp"
    szTmpSerializedPath = new wchar_t[tmpFilePathSize + 1];
    swprintf(
        szTmpSerializedPath, tmpFilePathSize + 1, L"%ws.tmp", szSerializedPath);
...
JsErrorCode ChakraHost::RunSerializedScript(
    const wchar_t *szPath,
    const wchar_t *szSerializedPath,
    const wchar_t *szSourceUri,
    JsValueRef *result) {
  HANDLE hFile = NULL;
  HANDLE hMap = NULL;
  BYTE *buffer = nullptr;
  ...
  __try {
    error = JsRunSerializedScriptWithCallback(
        &LoadSourceCallback,
    ...
    if (AbnormalTermination()) {
      // If JsRunSerializedScriptWithCallback threw any SEH exception (due to
      // corrupted bytecode file that can't be safely caught by Chakra engine),
      // we make sure we kill the bytecode so next run is fresh.
      context->Dispose();
      // This keeps the code clean even though we know app won't make it to the
      // next if block..
      context = NULL;
      _wunlink(szSerializedPath);
    }
  ...
  if (error != JsNoError) {
    // UnloadSourceCallback is called even in error case (though
    // LoadSourceCallback never is), so we can't fully delete the context
    if (context != NULL) {
      context->Dispose();
    }
    return error;
...
JsErrorCode ChakraHost::RunScript(
...
JsErrorCode ChakraHost::EvaluateScript(
    const wchar_t *szScript,
    const wchar_t *szSourceUri,
    JsValueRef *result) {
  if (szScript == nullptr) {
...
JsErrorCode ChakraHost::GetGlobalVariable(
    const wchar_t *szPropertyName,
    JsValueRef *result) {
...
JsErrorCode ChakraHost::SetGlobalVariable(
    const wchar_t *szPropertyName,
    JsValueRef value) {
  JsPropertyIdRef globalVarId;
  IfFailRet(JsGetPropertyIdFromName(szPropertyName, &globalVarId));
...
void ChakraHost::SetFlushQueueImmediate(
    ChakraBridge::FlushQueueImmediateHandler ^ handler) {
...
JsErrorCode ChakraHost::InitConsole() {
  IfFailRet(DefineHostCallback(
      globalObject, L"nativeLoggingHook", NativeLoggingCallback, nullptr));
...
JsErrorCode ChakraHost::InitNativeRequire() {
  IfFailRet(
      DefineHostCallback(globalObject, L"nativeRequire", NativeRequire, this));
...
JsErrorCode ChakraHost::InitNativeCallSyncHook() {
  IfFailRet(DefineHostCallback(
      globalObject, L"nativeCallSyncHook", NativeCallSyncHook, this));
