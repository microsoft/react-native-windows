...
void HttpSession::Respond()
{
  ...
  case http::verb::options:
    ...
    m_response->set(http::field::access_control_request_headers, "Access-Control-Allow-Headers, Content-type, Custom-Header, Header-expose-allowed");
...
void HttpServer::OnAccept(error_code ec)
{
  ...
  //Accept(); //ISS:2735328: Uncomment after implementing multiple context threading.
}
...
void HttpServer::Start()
{
  ...
  {
    // See https://www.boost.org/doc/libs/1_68_0/doc/html/boost_asio/reference/io_context/run/overload1.html
...
void HttpServer::SetOnGet(function<http::response<http::dynamic_body>(const http::request<http::string_body>&)>&& handler) noexcept
{
