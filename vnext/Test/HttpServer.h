...
struct HttpCallbacks
{
  ...
  std::function<boost::beast::http::response<boost::beast::http::dynamic_body>(const boost::beast::http::request<boost::beast::http::string_body>&)> OnGet;
};
...
// Accepts client connections and dispatches sessions for each incoming connection.
///
...
public:
  ...
  ///
  // Function that creates an HTTP response to send to the client on GET requests.
  ///
  void SetOnGet(std::function<boost::beast::http::response<boost::beast::http::dynamic_body>(const boost::beast::http::request<boost::beast::http::string_body>&)>&& onGet) noexcept;
};
