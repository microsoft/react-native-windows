...
template <typename Protocol, typename Socket, typename Resolver>
void LegacyBaseWebSocket<Protocol, Socket, Resolver>::Handshake(
...
template <typename Protocol, typename Socket, typename Resolver>
void LegacyBaseWebSocket<Protocol, Socket, Resolver>::EnqueueWrite(
    const std::string &message,
    bool binary) {
  m_requests.emplace(std::make_pair<std::string, bool>(
      std::move(std::string(message)), std::move(binary)));
...
LegacyBaseWebSocket<Protocol, Socket, Resolver>::ToBeastCloseCode(
...
template <typename Protocol, typename Socket, typename Resolver>
void LegacyBaseWebSocket<Protocol, Socket, Resolver>::Connect(
    const Protocols &protocols,
    const Options &options) {
  m_stream->control_callback(
      [this](websocket::frame_type type, boost::string_view view) {
        switch (type) {
...
template <typename Protocol, typename Socket, typename Resolver>
void LegacyBaseWebSocket<Protocol, Socket, Resolver>::Close(
    CloseCode code,
    const string &reason) {
  ...
  m_stream->async_close(
      ToBeastCloseCode(code), [this, code, reason](boostecr ec) {
        m_closing = true;
...
template <typename Protocol, typename Socket, typename Resolver>
void LegacyBaseWebSocket<Protocol, Socket, Resolver>::Send(
    const string &message) {
  EnqueueWrite(std::move(message), false);
}

template <typename Protocol, typename Socket, typename Resolver>
void LegacyBaseWebSocket<Protocol, Socket, Resolver>::SendBinary(
...
template <typename Protocol, typename Socket, typename Resolver>
void LegacyBaseWebSocket<Protocol, Socket, Resolver>::SetOnConnect(
    std::function<void()> &&handler) {
  m_connectHandler = handler;
}

template <typename Protocol, typename Socket, typename Resolver>
void LegacyBaseWebSocket<Protocol, Socket, Resolver>::SetOnPing(
    std::function<void()> &&handler) {
  m_pingHandler = handler;
}

template <typename Protocol, typename Socket, typename Resolver>
void LegacyBaseWebSocket<Protocol, Socket, Resolver>::SetOnSend(
    std::function<void(size_t)> &&handler) {
  m_writeHandler = handler;
}

template <typename Protocol, typename Socket, typename Resolver>
void LegacyBaseWebSocket<Protocol, Socket, Resolver>::SetOnMessage(
    std::function<void(size_t, const string &)> &&handler) {
  m_readHandler = handler;
}

template <typename Protocol, typename Socket, typename Resolver>
void LegacyBaseWebSocket<Protocol, Socket, Resolver>::SetOnClose(
    std::function<void(CloseCode, const std::string &)> &&handler) {
  m_closeHandler = handler;
}

template <typename Protocol, typename Socket, typename Resolver>
void LegacyBaseWebSocket<Protocol, Socket, Resolver>::SetOnError(
...
LegacyBaseWebSocket<Protocol, Socket, Resolver>::GetReadyState() const {
  return m_readyState;
}
...
template <typename Protocol, typename Socket, typename Resolver>
LegacySecureWebSocket<Protocol, Socket, Resolver>::LegacySecureWebSocket(
    Url &&url)
    : LegacyBaseWebSocket<Protocol, Socket, Resolver>(std::move(url)) {
...
template <typename Protocol, typename Socket, typename Resolver>
void LegacySecureWebSocket<Protocol, Socket, Resolver>::Handshake(
    const IWebSocket::Options &options) {
  this->m_stream->next_layer().async_handshake(
      ...
      [this, options = std::move(options)](boostecr ec) {
        if (ec && this->m_errorHandler) {
          ...
          LegacyBaseWebSocket<Protocol, Socket, Resolver>::Handshake(
              std::move(options));
        }
      });
}
...
/*static*/ std::unique_ptr<IWebSocket> IWebSocket::MakeLegacy(
  ...
  if (url.scheme == "ws") {
    ...
    return std::unique_ptr<IWebSocket>(
        new LegacyWebSocket<tcp>(std::move(url)));
  } else if (url.scheme == "wss") {
    if (url.port.empty())
      url.port = "443";

    return std::unique_ptr<IWebSocket>(
        new LegacySecureWebSocket<tcp, ssl::stream<tcp::socket>>(
            std::move(url)));
  } else
    throw std::exception(
