...
/*static*/ bool OriginPolicyHttpFilter::IsSimpleCorsRequest(HttpRequestMessage const &request) noexcept {
  ...
  /*static*/ bool OriginPolicyHttpFilter::IsCorsSafelistedRequestHeader(
      hstring const &name, hstring const &value) noexcept {
  ...
  /*static*/ void OriginPolicyHttpFilter::RemoveHttpOnlyCookiesFromResponseHeaders(
      HttpResponseMessage const &response, bool removeAll) {
  ...
  void OriginPolicyHttpFilter::ValidateAllowOrigin(
      hstring const &allowedOrigin, hstring const &allowCredentials, IMap<hstring, IInspectable> props) const {
    ...
    void OriginPolicyHttpFilter::ValidatePreflightResponse(
        HttpRequestMessage const &request, HttpResponseMessage const &response) const {
      ...
      for (const auto &method : controlValues.AllowedMethods) {
        ...
        requestMethodAllowed |=
            s_simpleCorsMethods.find(request.Method().ToString().c_str()) != s_simpleCorsMethods.cend();
    ...
    ResponseOperation OriginPolicyHttpFilter::SendPreflightAsync(HttpRequestMessage const &request) const {
      ...
      if (coRequest.Content()) {
        for (const auto &header : coRequest.Content().Headers()) {
          ...
          void OriginPolicyHttpFilter::ValidateResponse(
              HttpResponseMessage const &response, const OriginPolicy originPolicy) const {
            ...
            if (originPolicy == OriginPolicy::SimpleCrossOriginResourceSharing ||
              ...
              if (originPolicy == OriginPolicy::SimpleCrossOriginResourceSharing) {
                ...
                for (const auto &header : response.Headers().GetView()) {
                  if (s_simpleCorsResponseHeaderNames.find(header.Key().c_str()) ==
                      s_simpleCorsResponseHeaderNames.cend())
                ...
                if (withCredentials || controlValues.ExposedHeaders.find(L"*") == controlValues.ExposedHeaders.cend()) {
                  ...
                  for (const auto &header : response.Headers().GetView()) {
                    if (s_simpleCorsResponseHeaderNames.find(header.Key().c_str()) ==
                            s_simpleCorsResponseHeaderNames.cend() &&
                        controlValues.ExposedHeaders.find(header.Key().c_str()) ==
                            controlValues.ExposedHeaders.cend()) {
                ...
                if (originPolicy == OriginPolicy::SimpleCrossOriginResourceSharing) {
                  ...
                  // Keep simple headers and those found in the expose header list.
                  if (withCredentials ||
                      controlValues.ExposedHeaders.find(L"*") == controlValues.ExposedHeaders.cend()) {
                    std::queue<hstring> nonSimpleNonExposedHeaders;
                ...
                // When withCredentials is false, request cannot include cookies. Also, cookies will be ignored in
                // responses.
                removeAllCookies = !withCredentials && GetRuntimeOptionBool("Http.RemoveCookiesFromResponse");
              } // originPolicy == SimpleCrossOriginResourceSharing || CrossOriginResourceSharing
...
#pragma region IRedirectEventSource
            ...
            bool OriginPolicyHttpFilter::OnRedirecting(
                HttpRequestMessage const &request, HttpResponseMessage const &response) noexcept {
              // Consider the following scenario.
              // User signs in to http://a.com and visits a page that makes CORS request to http://b.com with
              // origin=http://a.com. Http://b.com reponds with a redirect to http://a.com. The browser follows the
              // redirect to http://a.com with origin=http://a.com. Since the origin matches the URL, the request is
              // authorized at http://a.com, but it actually allows http://b.com to bypass the CORS check at
              // http://a.com since the redirected URL is from http://b.com.
              if (!IsSameOrigin(response.Headers().Location(), request.RequestUri()) &&
                  !IsSameOrigin(s_origin, request.RequestUri())) {
                // By masking the origin field in the request header, we make it impossible for the server to set a
                // single value for the access-control-allow-origin header. It means, the only way to support redirect
                // is that server allows access from all sites through wildcard.
