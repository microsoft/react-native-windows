...
namespace facebook {
  namespace react {
    ...
    std::shared_ptr<const jsi::Buffer> BasePreparedScriptStoreImpl::tryGetPreparedScript(
      ...
      std::optional<std::vector<std::uint8_t>> hashBuffer = Microsoft::ReactNative::GetSHA256Hash(reinterpret_cast<const std::uint8_t*>(buffer->data()) + sizeof(PreparedScriptPrefix), static_cast<size_t>(prefix->sizeInBytes));
      if (!hashBuffer) {
        // Hashing failed.
        return nullptr;
      }
    ...
    void BasePreparedScriptStoreImpl::persistPreparedScript(
      ...
      std::optional<std::vector<std::uint8_t>> hashBuffer = Microsoft::ReactNative::GetSHA256Hash(preparedScript->data(), preparedScript->size());
      if (!hashBuffer) {
        // Hashing failed.
        std::terminate();
      }
