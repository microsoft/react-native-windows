...
bool URLValidator::IsOctalIPv4(const std::string& hostname) {
    if (hostname.empty() || hostname[0] != '0') return false;
...
bool URLValidator::IsPrivateOrLocalhost(const std::string& hostname) {
    ...
    if (hostname.find("10.") == 0 ||
        hostname.find("192.168.") == 0 ||
        hostname.find("127.") == 0) {
        return true;
    }
    ...
    if (hostname.find("fc00:") == 0 || hostname.find("fe80:") == 0 ||
        hostname.find("fd00:") == 0 || hostname.find("ff00:") == 0) {
        return true;
    }
...
const std::regex PathValidator::TRAVERSAL_REGEX(
    R"(\.\.|\\\\|\/\.\./|%2e%2e|%252e%252e|%5c|%255c)",
    std::regex::icase
);

const std::regex PathValidator::BLOB_ID_REGEX(
    R"(^[a-zA-Z0-9_-]{1,128}$)"
);
...
bool PathValidator::ContainsTraversal(const std::string& path) {
    ...
    if (std::regex_search(path, TRAVERSAL_REGEX) || 
        std::regex_search(decoded, TRAVERSAL_REGEX)) {
...
void SizeValidator::ValidateSize(
    size_t size,
    size_t maxSize,
    const char* context
) {
    if (size > maxSize) {
        ...
        oss << context << " size (" << size << " bytes) exceeds maximum ("
            << maxSize << " bytes)";
...
void SizeValidator::ValidateInt32Range(int32_t value, int32_t min, int32_t max, const char* context) {
    if (value < min || value > max) {
        std::ostringstream oss;
        oss << context << " value (" << value << ") outside valid range ["
            << min << ", " << max << "]";
...
const std::regex EncodingValidator::BASE64_REGEX(
    R"(^[A-Za-z0-9+/]*={0,2}$)"
);
...
bool EncodingValidator::ContainsCRLF(const std::string& str) {
    for (size_t i = 0; i < str.length(); ++i) {
        ...
        if (c == '%' && i + 2 < str.length()) {
            ...
            if (encoded == "%0D" || encoded == "%0d" || 
                encoded == "%0A" || encoded == "%0a") {
                return true;
            }
        }
    }
...
void EncodingValidator::ValidateHeaderValue(const std::string& value) {
    ...
    if (value.length() > SizeValidator::MAX_HEADER_LENGTH) {
        ...
        throw ValidationException("Header value exceeds maximum length (" + 
                                 std::to_string(SizeValidator::MAX_HEADER_LENGTH) + ")");
    }
