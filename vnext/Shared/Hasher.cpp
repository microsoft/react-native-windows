...
SHA256Hasher::SHA256Hasher() {
  ...
  DWORD cbData;
  if (!NT_SUCCESS(
          status = BCryptGetProperty(m_hAlg, BCRYPT_OBJECT_LENGTH, (PBYTE)&cbObject, sizeof(DWORD), &cbData, 0))) {
  ...
  if (!NT_SUCCESS(
          status = BCryptCreateHash(
              m_hAlg, &m_hHash, m_hashObject.data(), static_cast<ULONG>(m_hashObject.size()), NULL, 0, 0))) {
...
std::vector<std::uint8_t> SHA256Hasher::GetHashValue() {
  ...
  if (!NT_SUCCESS(
          status = BCryptGetProperty(m_hAlg, BCRYPT_HASH_LENGTH, (PBYTE)&hashLength, sizeof(DWORD), &cbData, 0))) {
