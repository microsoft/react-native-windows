...
#define	CheckNTSuccess(s) do { NTSTATUS status = (s); if (!NT_SUCCESS(status)) { throw status; } } while (false)
...
namespace Microsoft::ReactNative {
  ...
  SHA256Hasher::SHA256Hasher()
  {
    ...
    CheckNTSuccess(BCryptCreateHash(m_hAlg, &m_hHash, m_hashObject.data(), static_cast<ULONG>(m_hashObject.size()), NULL, 0, 0));
  }
