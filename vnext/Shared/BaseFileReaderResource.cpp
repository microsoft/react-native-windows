...
void BaseFileReaderResource::ReadAsText(string&& blobId, int64_t offset, int64_t size, string&& encoding, shared_ptr<IBlobPersistor> persistor, function<void(string&&)>&& resolver, function<void(string&&)>&& rejecter) noexcept /*override*/ {
...
void BaseFileReaderResource::ReadAsDataUrl(string&& blobId, int64_t offset, int64_t size, string&& type, shared_ptr<IBlobPersistor> persistor, function<void(string&&)>&& resolver, function<void(string&&)>&& rejecter) noexcept /*override*/ {
  if (!persistor) {
...
/*static*/ shared_ptr<IFileReaderResource> IFileReaderResource::Make() noexcept {
  return std::make_shared<BaseFileReaderResource>();
}
