...
namespace facebook {
  namespace react {
    StorageFileIO::StorageFileIO(const WCHAR* storageFileName)
    {
      ...
      const std::wstring strStorageFileFullPath = strStorageFolderFullPath + L"\\" + storageFileName + strStorageFileExtension;
      ...
      m_storageFileHandle = CreateFile2(strStorageFileFullPath.c_str(), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, OPEN_ALWAYS, &extendedParams);
#else
      m_storageFileHandle = CreateFileW(strStorageFileFullPath.c_str(), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr);
#endif
      ...
      m_storageFile = std::unique_ptr<FILE, std::function<void(FILE *)>>(_fdopen(fdFileDescriptor, "r+"), [](FILE *f) {fclose(f); });
    ...
    void StorageFileIO::throwLastErrorMessage()
    {
      ...
      OACR_REVIEWED_CALL(tudorm, FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM, nullptr, GetLastError(), MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), errorMessageBuffer, IOHelperBufferSize, nullptr));
#else
      FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM, nullptr, GetLastError(), MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), errorMessageBuffer, IOHelperBufferSize, nullptr);
#endif
