...
namespace facebook {
namespace react {
std::vector<string> FollyDynamicConverter::jsArgAsStringVector(
...
FollyDynamicConverter::jsArgAsTupleStringVector(const dynamic &args) noexcept {
  // args is array[array[array[]]]
  std::vector<tuple<string, string>> kVVector;
  const int iKeyPos = 0;
  const int iValuePos = 1;
  for (const auto &jsKV : args[0]) {
    kVVector.emplace_back(
        jsKV[iKeyPos].getString(), jsKV[iValuePos].getString());
  }
  return kVVector;
}

folly::dynamic FollyDynamicConverter::stringVectorAsRetVal(
...
folly::dynamic FollyDynamicConverter::tupleStringVectorAsRetVal(
  ...
  for (const auto &retVal : vec) {
    jsRetVals.push_back(
        dynamic::array(std::get<0>(retVal), std::get<1>(retVal)));
  }
  return jsRetVals;
}
} // namespace react
