...
void BatchingUIManager::processQueue() {
  ...
  _snprintf_s(
      ...
      _countof(buffer),
      _TRUNCATE,
      "BatchingUIManager Batches: %u Calls: %u (%u new)\r\n",
      ++cBatches,
      cCalls,
      cTheseCalls);
...
void BatchingUIManager::removeRootView(folly::dynamic &&args) {
  ...
  dispatchFunction(
      [manager, args]() { manager->removeRootView(jsArgAsInt(args, 0)); });
}

void BatchingUIManager::configureNextLayoutAnimation(
    folly::dynamic &&args,
...
void BatchingUIManager::createView(folly::dynamic &&args) {
  ...
  dispatchFunction([manager, args]() {
    ...
    static_cast<IUIManager *>(manager.get())
        ->createView(
            jsArgAsInt(args, 0),
...
void BatchingUIManager::setChildren(folly::dynamic &&args) {
  ...
  dispatchFunction([manager, args]() {
    ...
    static_cast<IUIManager *>(manager.get())
        ->setChildren(jsArgAsInt(args, 0), std::move(arg1));
  });
}
...
void BatchingUIManager::updateView(folly::dynamic &&args) {
  ...
  dispatchFunction([manager, args]() {
    ...
    static_cast<IUIManager *>(manager.get())
        ->updateView(
            jsArgAsInt(args, 0), jsArgAsString(args, 1), std::move(arg2));
  });
}

void BatchingUIManager::removeSubviewsFromContainerWithID(
    folly::dynamic &&args) {
  auto manager = shared_from_this();
  dispatchFunction([manager, args]() {
    static_cast<IUIManager *>(manager.get())
        ->removeSubviewsFromContainerWithID(jsArgAsInt(args, 0));
  });
}
...
void BatchingUIManager::manageChildren(folly::dynamic &&args) {
  ...
  dispatchFunction([manager, args]() {
    ...
    static_cast<IUIManager *>(manager.get())
        ->manageChildren(jsArgAsInt(args, 0), arg1, arg2, arg3, arg4, arg5);
  });
}
  ...
  dispatchFunction([manager, args]() {
    ...
    static_cast<IUIManager *>(manager.get())
        ->dispatchViewManagerCommand(
            jsArgAsInt(args, 0), jsArgAsInt(args, 1), std::move(arg2));
  });
}
...
void BatchingUIManager::replaceExistingNonRootView(folly::dynamic &&args) {
  ...
  dispatchFunction([manager, args]() {
    static_cast<IUIManager *>(manager.get())
        ->replaceExistingNonRootView(jsArgAsInt(args, 0), jsArgAsInt(args, 1));
  });
}

void BatchingUIManager::measure(
    folly::dynamic &&args,
    facebook::xplat::module::CxxModule::Callback cb) {
  auto manager = shared_from_this();
...
BatchingUIManagerModule::BatchingUIManagerModule(
...
BatchingUIManagerModule::getMethods() {
  ...
  return {
      ...
      Method(
          "createView",
          [manager](dynamic args) { manager->createView(std::move(args)); }),
      Method(
          "configureNextLayoutAnimation",
          [manager](dynamic args, Callback cbSuccess, Callback cbError) {
            manager->configureNextLayoutAnimation(
                std::move(args), cbSuccess, cbError);
          },
          AsyncTag),
      Method(
          "setChildren",
          [manager](dynamic args) { manager->setChildren(std::move(args)); }),
      Method(
          "updateView",
          [manager](dynamic args) { manager->updateView(std::move(args)); }),
      Method(
      ...
      Method(
          "measure",
          [manager](
              dynamic args, facebook::xplat::module::CxxModule::Callback cb) {
            manager->measure(std::move(args), cb);
          }),
      Method(
          "focus",
          [manager](dynamic args) { manager->focus(std::move(args)); }),
      Method(
          "blur", [manager](dynamic args) { manager->blur(std::move(args)); }),
      Method(
          "setJSResponder",
          [](dynamic args) {
            // TODO: Implement?
...
shared_ptr<IUIManager> createBatchingUIManager(
    std::vector<std::unique_ptr<IViewManager>> &&viewManagers,
    INativeUIManager *nativeManager) {
  return std::make_shared<BatchingUIManager>(
      std::move(viewManagers), nativeManager);
}

std::unique_ptr<facebook::xplat::module::CxxModule>
createBatchingUIManagerModule(std::shared_ptr<IUIManager> uimanager) {
