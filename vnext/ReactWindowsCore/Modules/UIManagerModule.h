...
class UIManager : public IUIManager, INativeUIManagerHost {
 public:
  ...
  void populateViewManagerConstants(
      std::map<std::string, folly::dynamic> &constants) override;
  ...
  void dispatchViewManagerCommand(
      int64_t reactTag,
      int64_t commandId,
      folly::dynamic &&commandArgs) override;
 ...
 private:
  ...
  void
  DropView(int64_t tag, bool removeChildren = true, bool zombieView = false);
