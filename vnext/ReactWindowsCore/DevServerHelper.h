...
using JSECreator = std::function<std::unique_ptr<JSExecutor>(std::shared_ptr<ExecutorDelegate>,
...
public:
	...
	static std::string get_BundleUrl(const std::string& debugHost, const std::string& jsbundle, const std::string& platform, const std::string& dev, const std::string& hot)
	{
		return string_format(BundleUrlFormat, GetDeviceLocalHost(debugHost).c_str(), jsbundle.c_str(), platform.c_str(), dev.c_str(), hot.c_str());
	}
