...
// deallocation. Thus, it is required to use primitives that ensure that object's
// memory is allocated and deallocated in the same DLL.
//
...
// Specialization of std::unique_ptr that uses AbiObjectDeleter instead of default deleter.
template <class T>
...
struct IAbiFunctor; // Intentionally not implemented to allow the specialization for function signatures below.
...
template <class TSignature>
struct IAbiFunctorThrow; // Intentionally not implemented to allow the specialization for function signatures below.
...
// The base class is not copyable because it must be always managed by std::unique_ptr.
