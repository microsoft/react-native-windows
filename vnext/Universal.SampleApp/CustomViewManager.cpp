...
/*static*/ void CustomFrameworkElementShadowNode::OnCheckedChanged(react::uwp::IReactInstance& instance, int64_t tag, bool newValue)
{
...
CustomFrameworkElementViewManager::CustomFrameworkElementViewManager(const std::shared_ptr<react::uwp::IReactInstance>& reactInstance)
  : Super(reactInstance)
{
}
...
folly::dynamic CustomFrameworkElementViewManager::GetCommands() const
{
  ...
  commands.update(folly::dynamic::object
    ("ExampleCommand", static_cast<std::underlying_type_t<CustomFrameworkElementCommands>>(CustomFrameworkElementCommands::ExampleCommand))
  );
  return commands;
}
...
void CustomFrameworkElementViewManager::UpdateProperties(react::uwp::ShadowNodeBase* nodeToUpdate, folly::dynamic reactDiffMap)
{
  ...
  for (auto& pair : reactDiffMap.items())
  {
    ...
    else if (propertyName == "test")
    {
      ...
      {
        // Always handle isNull, called when a property is unset, especially common with style props
      }
    }
  }
...
void CustomFrameworkElementViewManager::DispatchCommand(react::uwp::XamlView viewToUpdate, int64_t commandId, const folly::dynamic& commandArgs)
{
