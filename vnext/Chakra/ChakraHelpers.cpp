...
void serializeBytecodeToFileCore(const std::shared_ptr<const JSBigString>& script, const BytecodePrefix& bytecodePrefix, const std::string& bytecodeFileName)
{
...
void serializeBytecodeToFile(const std::shared_ptr<const JSBigString>& script, const BytecodePrefix& bytecodePrefix, std::string&& bytecodeFileName, bool async)
{
  std::future<void> bytecodeSerializationFuture = std::async(std::launch::async,
    [](const std::shared_ptr<const JSBigString>& script, const BytecodePrefix& bytecodePrefix, const std::string& bytecodeFileName)
    {
...
} // anonymous namespace within facebook::react
#endif

MinimalChakraRuntime::MinimalChakraRuntime(bool multithreaded)
  : runtime{ new JsRuntimeHandle, [](JsRuntimeHandle* h) { JsDisposeRuntime(*h); delete h; } }
  , context{ new JsContextRef, [](JsContextRef* c) { JsSetCurrentContext(JS_INVALID_REFERENCE); delete c; } }
{
    JsErrorCode lastError = JsCreateRuntime(multithreaded ? JsRuntimeAttributeNone : JsRuntimeAttributeDisableBackgroundWork, nullptr, &(*runtime));
  ...
  bool asyncBytecodeGeneration)
{
  ...
  // Hence for UWP implementation, we fall back to using the original source code right now.
  ...
  JsValueRef exn = nullptr;
  JsValueRef value = nullptr;
  JsErrorCode result = JsRunSerialized(jsArrayBufferFromBigString(std::move(bytecode)).get(),
    [](JsSourceContext sourceContext, JsValueRef *value, JsParseScriptAttributes *parseAttributes)
  {
    ...
    *parseAttributes = JsParseScriptAttributeNone;
    return true;
  }, reinterpret_cast<JsSourceContext>(jsScript.get()), scriptFileName, &value);

  // Currently, when the existing bundle.bytecode is incompatible with the ChakraCore.dll used,
  // we do not update it. This is because we memory mapped bundle.bytecode into a JsExternalArrayBuffer,
  // whose lifetime is controlled by the JS engine. Hence we cannot remove/rename/modify
  // bytecode.bundle until the JS garbage collector deletes the corresponding JsExternalArrayBuffer.
...
JsValueRef JSObjectCallAsFunction(JsValueRef methodJSRef, JsValueRef batchedBridgeRef, unsigned nArgs, const JsValueRef args[], JsValueRef* error)
{
  ...
  JsValueRef value;
  auto result = JsCallFunction(methodJSRef, const_cast<JsValueRef*>(argsWithThis), static_cast<unsigned short>(nArgs + 1), &value);
  delete[] argsWithThis;
  bool hasException = false;
...
{
  // Handle special case with Chakra runtime where if you pass an empty string to parseFunction it will throw an exception instead of returning nullptr
...
{
  // TODO exception handing -- ensure proper clean up if failure in middle of function
  JsPropertyIdRef propId = JS_INVALID_REFERENCE;
  std::string propName;
