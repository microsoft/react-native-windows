  ...
  struct funcWrapper
  {
      ...
      void *callbackState)
    {
      ...
      try
      {
        ...
        JsValueRef thisPropertyName = argumentCount > 1 ? arguments[1] : (JsGetNullValue(&thisPropertyName), thisPropertyName);
        ChakraExecutor* executor = nullptr;
        JsGetContextData(ctx, (void**)&executor);
...
ChakraExecutor::ChakraExecutor(std::shared_ptr<ExecutorDelegate> delegate,
  ...
  m_instanceArgs(std::move(instanceArgs))
{
...

  {
    ...
    // For Windows, we don't use nativeModuleProxy. Instead, we register the native module configs.
...
// This class tracks the created runtimes on a given thread, through the threadLocal tls_runtimeTracker variable
...
void ChakraExecutor::initOnJSVMThread()
{
  ...
  if (tls_runtimeTracker.RuntimeHandle == nullptr)
  {
    ...
    if (enableDebugging)
    {
      ...
      JsErrorCode result = this->enableDebugging(tls_runtimeTracker.RuntimeHandle, runtimeName, breakOnNextLine, port, tls_runtimeTracker.DebugProtocolHandler, tls_runtimeTracker.DebugService);
  ...
  // Although the lambda has access to tls_runtimeTracker, we have to call reportMemory(De)Allocation
  // through a pointer. This is because ChakraCore may invoke this callback on a different thread,
  // in which case tls_runtimeTracker will refer to a different thread local storage.
  JsSetRuntimeMemoryAllocationCallback(tls_runtimeTracker.RuntimeHandle, tls_runtimeTracker.MemoryTracker.get(),
...
void ChakraExecutor::loadApplicationScript(std::unique_ptr<const JSBigString> script,
#if !defined(OSS_RN)
...
{
  SystraceSection s("ChakraExecutor::loadApplicationScript",
    "sourceURL", sourceURL);
  ...
  // when debugging is enabled, don't use bytecode caching because ChakraCore doesn't support it.
#if !defined(OSS_RN)
...
void ChakraExecutor::callFunction(const std::string& moduleId, const std::string& methodId, const folly::dynamic& arguments)
{
  SystraceSection s("ChakraExecutor::callFunction");
  // Set the current context, since we are potentially running on the same thread as other contexts
  JSContextHolder ctx(m_context);
  ...
  auto result = [&]
  {
    try
    {
      return m_callFunctionReturnFlushedQueueJS->callAsFunction({ ChakraValue(ChakraString::createExpectingAscii(moduleId)),
...
void ChakraExecutor::setGlobalVariable(std::string propName, std::unique_ptr<const JSBigString> jsonValue)
{
  try
  {
    SystraceSection s("ChakraExecutor.setGlobalVariable",
      "propName", propName);
  ...
  const JsValueRef arguments[])
{
  ...
  {
    // If there is no executor-specific logging callback, redirect to the global logging hook
