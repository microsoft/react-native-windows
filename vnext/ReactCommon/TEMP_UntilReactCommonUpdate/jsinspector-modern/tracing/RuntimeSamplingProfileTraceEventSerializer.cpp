...
HighResTimeStamp getHighResTimeStampForSample(const RuntimeSamplingProfile::Sample &sample) {
  auto microsecondsSinceSteadyClockEpoch = sample.timestamp;
  auto chronoTimePoint =
      std::chrono::steady_clock::time_point(std::chrono::microseconds(microsecondsSinceSteadyClockEpoch));
...
TraceEventProfileChunk::CPUProfile::Node convertToTraceEventProfileNode(const ProfileTreeNode &node) {
  const RuntimeSamplingProfile::SampleCallStackFrame &callFrame = node.getCallFrame();
  auto traceEventCallFrame = TraceEventProfileChunk::CPUProfile::Node::CallFrame{
      .codeType = node.getCodeType() == ProfileTreeNode::CodeType::JavaScript ? "JS" : "other",
      .scriptId = callFrame.scriptId,
      .functionName = std::string(callFrame.functionName),
      .url = callFrame.scriptURL ? std::optional<std::string>(std::string(*callFrame.scriptURL)) : std::nullopt,
  ...
  return TraceEventProfileChunk::CPUProfile::Node{
      ...
      .parentId = node.hasParent() ? std::optional<uint32_t>(node.getParentId()) : std::nullopt,
  };
}

RuntimeSamplingProfile::SampleCallStackFrame createArtificialCallFrame(std::string_view callFrameName) {
...
RuntimeSamplingProfile::SampleCallStackFrame createGarbageCollectorCallFrame() {
  return RuntimeSamplingProfile::SampleCallStackFrame{
      .kind = RuntimeSamplingProfile::SampleCallStackFrame::Kind::GarbageCollector,
      .scriptId = FALLBACK_SCRIPT_ID,
...
class ProfileTreeRootNode : public ProfileTreeNode {
 public:
  explicit ProfileTreeRootNode(uint32_t id)
      : ProfileTreeNode(id, CodeType::Other, createArtificialCallFrame(ROOT_FRAME_NAME)) {}
};

struct ProfileChunk {
  ProfileChunk(uint16_t chunkSize, ProcessId chunkProcessId, ThreadId chunkThreadId, HighResTimeStamp chunkTimestamp)
      : size(chunkSize), processId(chunkProcessId), threadId(chunkThreadId), timestamp(chunkTimestamp) {
...
void sendProfileTraceEvent(
  ...
  auto traceEvent =
      PerformanceTracer::constructRuntimeProfileTraceEvent(profileId, processId, threadId, profileStartTimestamp);
  folly::dynamic serializedTraceEvent = TraceEventSerializer::serialize(std::move(traceEvent));

  dispatchCallback(folly::dynamic::array(std::move(serializedTraceEvent)));
}

// Add an empty sample to the chunk.
void chunkEmptySample(ProfileChunk &chunk, uint32_t idleNodeId, HighResDuration samplesTimeDelta) {
...
// Event and buffer it.
void bufferProfileChunkTraceEvent(ProfileChunk &&chunk, RuntimeProfileId profileId, folly::dynamic &traceEventBuffer) {
      ...
      TraceEventProfileChunk{
          .cpuProfile =
              TraceEventProfileChunk::CPUProfile{
                  .nodes = std::move(traceEventNodes), .samples = std::move(chunk.samples)},
          .timeDeltas = std::move(chunk.timeDeltas),
      });
  auto serializedTraceEvent = TraceEventSerializer::serialize(std::move(traceEvent));
...
void processCallStack(
  ...
  for (auto it = callStack.rbegin(); it != callStack.rend(); ++it) {
    ...
    bool isGarbageCollectorFrame =
        callFrame.kind == RuntimeSamplingProfile::SampleCallStackFrame::Kind::GarbageCollector;

    ProfileTreeNode::CodeType childCodeType =
        isGarbageCollectorFrame ? ProfileTreeNode::CodeType::Other : ProfileTreeNode::CodeType::JavaScript;
    ...
    ProfileTreeNode *maybeExistingChild = previousNode->getIfAlreadyExists(childCodeType, childCallFrame);
    if (maybeExistingChild != nullptr) {
      previousNode = maybeExistingChild;
    } else {
      previousNode = previousNode->addChild(nodeIdGenerator.getNext(), childCodeType, childCallFrame);
...
/* static */ void RuntimeSamplingProfileTraceEventSerializer::serializeAndDispatch(
...
/* static */ void RuntimeSamplingProfileTraceEventSerializer::serializeAndDispatch(
  ...
  auto profileId = static_cast<RuntimeProfileId>(profileIdGenerator.getNext());// [Windows #15256]

  sendProfileTraceEvent(profile.processId, threadId, profileId, tracingStartTime, dispatchCallback);

  // There could be any number of new nodes in this chunk. Empty if all nodes
  // are already emitted in previous chunks.
  ProfileChunk chunk{profileChunkSize, profile.processId, threadId, currentChunkTimestamp};
  ...
  ProfileTreeNode *programNode = rootNode.addChild(
      nodeIdGenerator.getNext(), ProfileTreeNode::CodeType::Other, createArtificialCallFrame(PROGRAM_FRAME_NAME));
  ...
  for (auto &sample : samples) {
    ...
    if (currentSampleThreadId != chunk.threadId || chunk.isFull()) {
      ...
      chunk = ProfileChunk{profileChunkSize, profile.processId, currentSampleThreadId, currentChunkTimestamp};
    }
