...
/* static */ std::shared_ptr<TurboModule> DefaultTurboModules::getTurboModule(
  ...
  // [Windows] The C++ MutationObserver native module and the shared C++
  // Animated backend (AnimatedModule / NativeAnimatedNodesManagerProvider) are
  // not built on Windows. React Native Windows ships its own Animated
  // implementation and the corresponding feature flags
  // (enableMutationObserverByDefault, cxxNativeAnimatedEnabled,
  // useSharedAnimatedBackend) default to false, so the upstream registrations
  // are omitted here to avoid pulling their unused backends into the build.
