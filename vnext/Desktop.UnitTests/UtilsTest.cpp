...
{
  void ExpectUrl(string urlString, string protocol, string host, string port = "", string path = "/", string query = "")
  {
    Url url(urlString);
  ...
  {
    ExpectUrl("ws://example.com?a=1&b=2", "ws", "example.com", "", "/", "a=1&b=2");
  }
  ...
  {
    ExpectUrl("ws://example.com/the/nested/path/?", "ws", "example.com", "", "/the/nested/path/");
  }

  TEST_METHOD(UtilsTest_NestedSubdomain)
  {
    ExpectUrl("ws://nested.sub.domain.of.example.com", "ws", "nested.sub.domain.of.example.com");
  }
