...
// V8 doesn't currently pass all the tests (particularly around hostobjects), move those out into ChakraRuntimeUnitTests
// for now
TEST_CLASS (ChakraRuntimeUnitTest) {
 private:
 ...
 public:
  ChakraRuntimeUnitTest() : m_factory{runtimeGenerators()[0]}, m_runtimePtr{m_factory()}, rt{*m_runtimePtr} {}
 ...
 private:
  ...
  TEST_METHOD(HostObjectTest) {
    ...
    // TODO: Fix compilation ("deleted" function)
...
TEST_CLASS (JsiRuntimeUnitTests) {
 ...
 public:
#if defined(USE_V8)
  JsiRuntimeUnitTests() : m_factory{getV8Runtime()}, m_runtimePtr{m_factory()}, rt{*m_runtimePtr} {}
#else
 ...
 private:
  ...
  TEST_METHOD(FunctionConstructorTest) {
  ...
  TEST_METHOD(ExceptionStackTraceTest) {
  ...
  TEST_METHOD(JSErrorsArePropagatedNicely) {
  ...
  TEST_METHOD(JSErrorsCanBeConstructedWithStack) {
    auto err = JSError(rt, "message", "stack");
    Assert::AreEqual(err.getMessage(), {"message"});
    Assert::AreEqual(err.getStack(), {"stack"});
  }

  TEST_METHOD(ScopeDoesNotCrashTest) {
    Scope scope(rt);
    Object o(rt);
  }

  TEST_METHOD(ScopeDoesNotCrashWhenValueEscapes) {
    Value v;
  ...
  TEST_METHOD(HostObjectWithValueMembers) {
