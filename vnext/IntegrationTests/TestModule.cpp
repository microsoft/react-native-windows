...
vector<module::CxxModule::Method> TestModule::getMethods()
{
  return
  {
    Method("sendAppEvent", [this](dynamic args)
    {
      this->sendAppEvent(jsArgAsString(args, 0)); //TODO: 2nd arg?
    }),
      Method("markTestCompleted", [this](dynamic args)
    {
      this->markTestCompleted();
    }),
      Method("markTestPassed", [this](dynamic args)
    {
      this->markTestPassed(jsArgAsBool(args, 0));
    }),
      Method("shouldResolve", [this](dynamic args)
	  {
		  shouldResolve();
    }),
  };
