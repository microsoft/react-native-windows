...
void CheckBoxShadowNode::updateProperties(winrt::Microsoft::ReactNative::JSValueObject &props) {
  /*m_updating = true;
...
CheckBoxViewManager::CheckBoxViewManager(const Mso::React::IReactContext &context) : Super(context) {}

const wchar_t *CheckBoxViewManager::GetName() const {
  return L"AndroidCheckBox";
}

void CheckBoxViewManager::GetNativeProps(
    const winrt::Microsoft::ReactNative::IJSValueWriter &writer) const {
  //auto props = Super::GetNativeProps(writer);

  //props.update(folly::dynamic::object("on", "boolean")("enabled", "boolean"));
}

ShadowNode *CheckBoxViewManager::createShadow() const {
...
YGMeasureFunc CheckBoxViewManager::GetYogaCustomMeasureFunc() const {
