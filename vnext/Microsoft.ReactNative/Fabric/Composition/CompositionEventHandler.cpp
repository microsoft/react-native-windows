            ...
            if (auto strongRootView = strongThis->m_wkRootView.get()) {
              ...
              if (focusedComponent) {
                ...
                auto contextMenuArgs = winrt::make<
                    winrt::Microsoft::ReactNative::Composition::Input::implementation::ContextMenuRoutedEventArgs>(
                    tag, position, true /*isKeyboardTriggered*/);
                winrt::get_self<winrt::Microsoft::ReactNative::implementation::ComponentView>(focusedComponent)
                    ->OnContextMenu(contextMenuArgs);
                args.Handled(true);
              }
            }
          }
    ...
    case WM_CONTEXTMENU: {
      ...
      if (auto strongRootView = m_wkRootView.get()) {
        ...
        if (focusedComponent) {
          ...
          auto tag =
              winrt::get_self<winrt::Microsoft::ReactNative::implementation::ComponentView>(focusedComponent)->Tag();
          auto contextMenuArgs = winrt::make<
              winrt::Microsoft::ReactNative::Composition::Input::implementation::ContextMenuRoutedEventArgs>(
              tag, position, isKeyboardTriggered);
          winrt::get_self<winrt::Microsoft::ReactNative::implementation::ComponentView>(focusedComponent)
              ->OnContextMenu(contextMenuArgs);
