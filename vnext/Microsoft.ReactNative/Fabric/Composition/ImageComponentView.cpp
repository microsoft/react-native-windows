...
namespace Microsoft::ReactNative {
  ...
  void ImageComponentView::updateProps(
    ...
    if (oldImageProps.backgroundColor != newImageProps.backgroundColor
      || oldImageProps.blurRadius != newImageProps.blurRadius
      || oldImageProps.tintColor != newImageProps.tintColor
  ...
  void ImageComponentView::ensureDrawingSurface() noexcept {
    ...
    if (!m_drawingSurface && m_wicbmp) {
      ...
      case facebook::react::ImageResizeMode::Repeat:
        // TODO - set AlignmentRatio back to 0.5f when switching between resizeModes once we no longer recreate the drawing surface on prop changes. 
    ...
    if (CheckForDeviceRemoved(drawingSurfaceInterop->BeginDraw(d2dDeviceContext.put(), &offset))) {
      ...
      if (useEffects) {
        ...
        if (imageProps->blurRadius > 0) {
          ...
          winrt::check_hresult(
            gaussianBlurEffect->SetValue(D2D1_GAUSSIANBLUR_PROP_STANDARD_DEVIATION, (imageProps->blurRadius) / radiusMultiplier));
