...
Float WindowsTextInputShadowNode::baseline(const LayoutContext &layoutContext, Size size) const {
  AttributedString attributedString = getMostRecentAttributedString();
  ...
  auto top = YGNodeLayoutGetBorder(&yogaNode_, YGEdgeTop) + YGNodeLayoutGetPadding(&yogaNode_, YGEdgeTop);

  return textLayoutManager_->baseline(attributedString, getConcreteProps().paragraphAttributes, size) + top;
}
