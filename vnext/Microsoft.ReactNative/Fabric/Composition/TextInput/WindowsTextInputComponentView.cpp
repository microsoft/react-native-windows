...
void WindowsTextInputComponentView::textAlignOnUpdateProps(const std::string &newTextAlignment) noexcept {
  PARAFORMAT2 pf = {0};
  pf.cbSize = sizeof(PARAFORMAT2);
  pf.dwMask = PFM_ALIGNMENT;

  auto &props = windowsTextInputProps();

  if (newTextAlignment == "center") {
    pf.wAlignment = PFA_CENTER;
  } else if (newTextAlignment == "right") {
    pf.wAlignment = PFA_RIGHT;
  } else {
    pf.wAlignment = PFA_LEFT;
  }

  MessageBox(NULL, reinterpret_cast<LPCWSTR>(std::to_string((pf.wAlignment)).c_str()), L"Debug", MB_OK);

  PARAFORMAT2 pf2 = {0};
  pf2.cbSize = sizeof(PARAFORMAT2);

  // Query the current paragraph format
  /* HRESULT hr = m_textServices->TxSendMessage(EM_GETPARAFORMAT, 0, (LPARAM)&pf2, nullptr);
  //m_textServices->TxSendMessage(EM_GETPARAFORMAT, 0, (LPARAM)&pf2, nullptr);

  if (SUCCEEDED(hr)) {
    if (pf2.dwMask & PFM_ALIGNMENT) {
      switch (pf2.wAlignment) {
        case PFA_LEFT:
          MessageBox(NULL, L"Alignment: Left", L"Debug", MB_OK);
          break;
        case PFA_CENTER:
          MessageBox(NULL, L"Alignment: Center", L"Debug", MB_OK);
          break;
        case PFA_RIGHT:
          MessageBox(NULL, L"Alignment: Right", L"Debug", MB_OK);
          break;
      }
    }
  }

  m_textServices->TxSendMessage(EM_SETPARAFORMAT, 0, (LPARAM)&pf, nullptr);
  HRESULT hr2 = m_textServices->TxSendMessage(EM_SETPARAFORMAT, 0, (LPARAM)&pf, nullptr);
  MessageBox(NULL, reinterpret_cast<LPCWSTR>(std::to_string((hr2)).c_str()), L"Debug", MB_OK);

  PARAFORMAT2 pf3 = {0};
  pf3.cbSize = sizeof(PARAFORMAT2);

  // Query the current paragraph format
  /* HRESULT hr3 = m_textServices->TxSendMessage(EM_GETPARAFORMAT, 0, (LPARAM)&pf3, nullptr);
  m_textServices->TxSendMessage(EM_GETPARAFORMAT, 0, (LPARAM)&pf3, nullptr);

  //DrawText();

  if (SUCCEEDED(hr3)) {
    if (pf3.dwMask & PFM_ALIGNMENT) {
      switch (pf3.wAlignment) {
        case PFA_LEFT:
          MessageBox(NULL, L"Alignment: Left", L"Debug", MB_OK);
          break;
        case PFA_CENTER:
