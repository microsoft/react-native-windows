...
void WindowsModalHostComponentView::EnsureModalCreated() {
  auto host = winrt::Microsoft::ReactNative::implementation::ReactNativeHost::GetReactNativeHost(m_context.Properties());
  ...
  // this is the name of the component registered in Javascript via AppRegistry.registerComponent('ModuleName', () => ModuleName);
  ...
  m_compositionHwndHost.ReactViewHost(winrt::Microsoft::ReactNative::ReactCoreInjection::MakeViewHost(host, viewOptions)); // creates a new ReactViewHost. What is the difference between CompositionHwndHost and ViewHost?
  ...
  // get the root hwnd
  auto roothwnd = reinterpret_cast<HWND>(winrt::Microsoft::ReactNative::ReactCoreInjection::GetTopLevelWindowId(m_context.Properties().Handle()));
...
void WindowsModalHostComponentView::MountChildComponentView(
  ...
  // Set parent: This line sets the parent of the childComponentView to *this (the current instance of WindowsModalHostComponentView). The get_self function is used to obtain the implementation object from the WinRT projection object.
  winrt::get_self<winrt::Microsoft::ReactNative::implementation::ComponentView>(childComponentView)->parent(*this);
  // Adjust index for borders: This line calls the indexOffsetForBorder method with the index parameter. This method likely adjusts the index to account for any special border handling or offsets required when inserting the child.
  indexOffsetForBorder(index);
  // Ensure visual setup: This line calls the ensureVisual method. This method probably ensures that the visual tree for the component view is properly set up and initialized.
  ensureVisual();
