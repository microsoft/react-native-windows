...
void AbiPortalShadowNode::layout(facebook::react::LayoutContext layoutContext) {
  ...
  portalOwningShadowNode = cloneTree(
      childNode->getFamily(),
      [&](const ShadowNode& oldShadowNode) {
        clonedShadowNode = oldShadowNode.clone({});
        return clonedShadowNode;
      });
  auto portalShadowNode =
      static_cast<AbiPortalShadowNode*>(portalOwningShadowNode.get());
  ...
  // Update the list of children to reflect the changes that we made.
    this->children_ =
      static_cast<AbiPortalShadowNode*>(portalOwningShadowNode.get())->children_;
}
