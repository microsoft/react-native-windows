...
shared_ptr<ITestInstance> TestRunner::GetInstance(
  ...
  viewManagers.push_back(
      unique_ptr<IViewManager>(new TestViewManager("RCTView")));
  viewManagers.push_back(
      unique_ptr<IViewManager>(new TestViewManager("RCTText")));
  viewManagers.push_back(
      unique_ptr<IViewManager>(new TestViewManager("RCTRawText")));
  viewManagers.push_back(
      unique_ptr<IViewManager>(new TestViewManager("RCTScrollView")));

  auto uiManager =
      createIUIManager(move(viewManagers), new TestNativeUIManager());
  auto nativeQueue = make_shared<TestMessageQueueThread>();
  auto jsQueue = make_shared<TestMessageQueueThread>();

  devSettings->jsiRuntimeHolder = std::make_shared<ChakraRuntimeHolder>(
      ...
      extraModules{
          make_tuple(
              ...
              []() -> unique_ptr<CxxModule> {
                return CreateAsyncStorageModule(L"ReactNativeAsyncStorage");
              },
              nativeQueue),
          make_tuple(
              "WebSocketModule",
          ...
          make_tuple(
              "Timing",
              [nativeQueue]() -> unique_ptr<CxxModule> {
                return CreateTimingModule(nativeQueue);
              },
              nativeQueue),
          ...
          make_tuple(
              AppStateModule::name,
              []() -> unique_ptr<CxxModule> {
                return make_unique<AppStateModule>(make_unique<AppState>());
              },
              nativeQueue),
          // Apparently mandatory for /IntegrationTests
          make_tuple(
