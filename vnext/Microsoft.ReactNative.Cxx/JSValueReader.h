...
inline bool ReadValue(winrt::Microsoft::ReactNative::Bridge::IJSValueReader const &reader, int16_t &value) noexcept {
  int64_t result{};
  return reader.TryGetInt64(result) && (value = static_cast<int16_t>(result), true);
}

inline bool ReadValue(winrt::Microsoft::ReactNative::Bridge::IJSValueReader const &reader, int32_t &value) noexcept {
  int64_t result{};
  return reader.TryGetInt64(result) && (value = static_cast<int32_t>(result), true);
}
  ...
  if constexpr (sizeof...(args) > 0) {
    ...
    if (state == JSValueReaderState::ArrayBegin) {
      ...
      [[maybe_unused]] int dummy[] = {
          (state = reader.ReadNext(), state != JSValueReaderState::ArrayEnd && ReadValue(reader, args), 0)...};
    }
  }
}
