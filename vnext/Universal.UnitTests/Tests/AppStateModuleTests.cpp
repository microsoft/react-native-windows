...
TEST_CLASS (AppStateModuleTest) {
  TEST_METHOD(AppStateModule_Initialize) {
    ...
    Callback first(
        [&](std::vector<folly::dynamic> args) { Assert::AreEqual(args[0]["app_state"].asString().c_str(), "active"); });
    Callback second;
