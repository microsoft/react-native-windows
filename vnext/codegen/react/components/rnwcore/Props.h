...
using RCTFlyoutPlacementMask = uint32_t;

enum class RCTFlyoutPlacement: RCTFlyoutPlacementMask {
  Top = 1 << 0,
  Bottom = 1 << 1,
  Left = 1 << 2,
  Right = 1 << 3,
  Full = 1 << 4,
  TopEdgeAlignedLeft = 1 << 5,
  TopEdgeAlignedRight = 1 << 6,
  BottomEdgeAlignedLeft = 1 << 7,
  BottomEdgeAlignedRight = 1 << 8,
  LeftEdgeAlignedTop = 1 << 9,
  RightEdgeAlignedTop = 1 << 10,
  LeftEdgeAlignedBottom = 1 << 11,
  RightEdgeAlignedBottom = 1 << 12
};

constexpr bool operator&(
  RCTFlyoutPlacementMask const lhs,
  enum RCTFlyoutPlacement const rhs) {
  return lhs & static_cast<RCTFlyoutPlacementMask>(rhs);
}

constexpr RCTFlyoutPlacementMask operator|(
  RCTFlyoutPlacementMask const lhs,
  enum RCTFlyoutPlacement const rhs) {
  return lhs | static_cast<RCTFlyoutPlacementMask>(rhs);
}
...
constexpr void operator|=(
  RCTFlyoutPlacementMask &lhs,
  enum RCTFlyoutPlacement const rhs) {
  lhs = lhs | static_cast<RCTFlyoutPlacementMask>(rhs);
}

static inline void fromRawValue(const PropsParserContext& context, const RawValue &value, RCTFlyoutPlacementMask &result) {
  auto items = std::vector<std::string>{value};
  for (const auto &item : items) {
    if (item == "top") {
      result |= RCTFlyoutPlacement::Top;
      continue;
    }
    if (item == "bottom") {
      result |= RCTFlyoutPlacement::Bottom;
      continue;
    }
    if (item == "left") {
      result |= RCTFlyoutPlacement::Left;
      continue;
    }
    if (item == "right") {
      result |= RCTFlyoutPlacement::Right;
      continue;
    }
    if (item == "full") {
      result |= RCTFlyoutPlacement::Full;
      continue;
    }
    if (item == "top-edge-aligned-left") {
      result |= RCTFlyoutPlacement::TopEdgeAlignedLeft;
      continue;
    }
    if (item == "top-edge-aligned-right") {
      result |= RCTFlyoutPlacement::TopEdgeAlignedRight;
      continue;
    }
    if (item == "bottom-edge-aligned-left") {
      result |= RCTFlyoutPlacement::BottomEdgeAlignedLeft;
      continue;
    }
    if (item == "bottom-edge-aligned-right") {
      result |= RCTFlyoutPlacement::BottomEdgeAlignedRight;
      continue;
    }
    if (item == "left-edge-aligned-top") {
      result |= RCTFlyoutPlacement::LeftEdgeAlignedTop;
      continue;
    }
    if (item == "right-edge-aligned-top") {
      result |= RCTFlyoutPlacement::RightEdgeAlignedTop;
      continue;
    }
    if (item == "left-edge-aligned-bottom") {
      result |= RCTFlyoutPlacement::LeftEdgeAlignedBottom;
      continue;
    }
    if (item == "right-edge-aligned-bottom") {
      result |= RCTFlyoutPlacement::RightEdgeAlignedBottom;
      continue;
    }
    abort();
  }
...
static inline std::string toString(const RCTFlyoutPlacementMask &value) {
    auto result = std::string{};
    auto separator = std::string{", "};

    if (value & RCTFlyoutPlacement::Top) {
      result += "top" + separator;
    }
    if (value & RCTFlyoutPlacement::Bottom) {
      result += "bottom" + separator;
    }
    if (value & RCTFlyoutPlacement::Left) {
      result += "left" + separator;
    }
    if (value & RCTFlyoutPlacement::Right) {
      result += "right" + separator;
    }
    if (value & RCTFlyoutPlacement::Full) {
      result += "full" + separator;
    }
    if (value & RCTFlyoutPlacement::TopEdgeAlignedLeft) {
      result += "top-edge-aligned-left" + separator;
    }
    if (value & RCTFlyoutPlacement::TopEdgeAlignedRight) {
      result += "top-edge-aligned-right" + separator;
    }
    if (value & RCTFlyoutPlacement::BottomEdgeAlignedLeft) {
      result += "bottom-edge-aligned-left" + separator;
    }
    if (value & RCTFlyoutPlacement::BottomEdgeAlignedRight) {
      result += "bottom-edge-aligned-right" + separator;
    }
    if (value & RCTFlyoutPlacement::LeftEdgeAlignedTop) {
      result += "left-edge-aligned-top" + separator;
    }
    if (value & RCTFlyoutPlacement::RightEdgeAlignedTop) {
      result += "right-edge-aligned-top" + separator;
    }
    if (value & RCTFlyoutPlacement::LeftEdgeAlignedBottom) {
      result += "left-edge-aligned-bottom" + separator;
    }
    if (value & RCTFlyoutPlacement::RightEdgeAlignedBottom) {
      result += "right-edge-aligned-bottom" + separator;
    }
    if (!result.empty()) {
      result.erase(result.length() - separator.length());
    }
...
using RCTFlyoutShowModeMask = uint32_t;

enum class RCTFlyoutShowMode: RCTFlyoutShowModeMask {
  Auto = 1 << 0,
  Standard = 1 << 1,
  Transient = 1 << 2,
  TransientWithDismissOnPointerMoveAway = 1 << 3
};

constexpr bool operator&(
  RCTFlyoutShowModeMask const lhs,
  enum RCTFlyoutShowMode const rhs) {
  return lhs & static_cast<RCTFlyoutShowModeMask>(rhs);
}

constexpr RCTFlyoutShowModeMask operator|(
  RCTFlyoutShowModeMask const lhs,
  enum RCTFlyoutShowMode const rhs) {
  return lhs | static_cast<RCTFlyoutShowModeMask>(rhs);
}
...
constexpr void operator|=(
  RCTFlyoutShowModeMask &lhs,
  enum RCTFlyoutShowMode const rhs) {
  lhs = lhs | static_cast<RCTFlyoutShowModeMask>(rhs);
}

static inline void fromRawValue(const PropsParserContext& context, const RawValue &value, RCTFlyoutShowModeMask &result) {
  auto items = std::vector<std::string>{value};
  for (const auto &item : items) {
    if (item == "auto") {
      result |= RCTFlyoutShowMode::Auto;
      continue;
    }
    if (item == "standard") {
      result |= RCTFlyoutShowMode::Standard;
      continue;
    }
    if (item == "transient") {
      result |= RCTFlyoutShowMode::Transient;
      continue;
    }
    if (item == "transient-with-dismiss-on-pointer-move-away") {
      result |= RCTFlyoutShowMode::TransientWithDismissOnPointerMoveAway;
      continue;
    }
    abort();
  }
...
static inline std::string toString(const RCTFlyoutShowModeMask &value) {
    auto result = std::string{};
    auto separator = std::string{", "};

    if (value & RCTFlyoutShowMode::Auto) {
      result += "auto" + separator;
    }
    if (value & RCTFlyoutShowMode::Standard) {
      result += "standard" + separator;
    }
    if (value & RCTFlyoutShowMode::Transient) {
      result += "transient" + separator;
    }
    if (value & RCTFlyoutShowMode::TransientWithDismissOnPointerMoveAway) {
      result += "transient-with-dismiss-on-pointer-move-away" + separator;
    }
    if (!result.empty()) {
      result.erase(result.length() - separator.length());
    }
...
#pragma mark - Props
  ...
  bool isOpen{false};
  RCTFlyoutPlacementMask placement{static_cast<RCTFlyoutPlacementMask>(RCTFlyoutPlacement::Top)};
  RCTFlyoutShowModeMask showMode{static_cast<RCTFlyoutShowModeMask>(RCTFlyoutShowMode::Auto)};
  int target{0};
