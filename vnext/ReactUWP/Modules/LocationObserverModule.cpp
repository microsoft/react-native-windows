...
{
public:
  LocationObserver(LocationObserverModule* parent, const std::shared_ptr<facebook::react::MessageQueueThread>& defaultQueueThread)
    : m_parent(parent)
...
static folly::dynamic ObjFromGeopos(winrt::Windows::Devices::Geolocation::Geoposition& geopos)
{
  ...
  // Since we can't pass NaN via json, don't include optional bearing/speed for now when stationary
...
winrt::Windows::Foundation::IAsyncOperation<winrt::Windows::Devices::Geolocation::Geoposition> GetGeoposition(winrt::Windows::Devices::Geolocation::Geolocator &geoLocator, LocationObserverOptions& options)
{
  auto location = co_await geoLocator.GetGeopositionAsync(TimeSpanFromMs(options.maxAge), TimeSpanFromMs(options.timeout));
  return location;
}

void LocationObserverModule::LocationObserver::getCurrentPosition(const folly::dynamic& optionsIn, Callback successCallback, Callback errorCallback)
{
  LocationObserverOptions options(optionsIn);

  winrt::Windows::Devices::Geolocation::Geolocator geoLocator;
  geoLocator.DesiredAccuracy(options.highAccuracy ? winrt::Windows::Devices::Geolocation::PositionAccuracy::High : winrt::Windows::Devices::Geolocation::PositionAccuracy::Default);
  ...
  catch (winrt::hresult_error const & e)
  {
    errorCallback({
      folly::dynamic::object
      ("code", 1)
      ("message", "Error in GetGeoposition:" + facebook::react::UnicodeConversion::Utf16ToUtf8(e.message())) });
    return;
  }
...
LocationObserverModule::LocationObserverModule(const std::shared_ptr<facebook::react::MessageQueueThread>& defaultQueueThread)
...
auto LocationObserverModule::getMethods() -> std::vector<Method>
{
  ...
  return {
    Method("getCurrentPosition", [locationObserver](folly::dynamic args, Callback successCallback, Callback errorCallback)
    {
      locationObserver->getCurrentPosition(facebook::xplat::jsArgAsDynamic(args, 0), successCallback, errorCallback);
    }, AsyncTag),
    Method("startObserving", [locationObserver](folly::dynamic args)
    {
      locationObserver->startObserving(facebook::xplat::jsArgAsDynamic(args, 0));
    }),
    Method("stopObserving", [locationObserver]()
    {
      locationObserver->stopObserving();
    }),
    Method("requestAuthorization", [locationObserver]()
    {
