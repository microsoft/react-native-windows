...
namespace react { namespace uwp {
  ...
  std::tuple<winrt::CompositionAnimation, winrt::CompositionScopedBatch> DecayAnimationDriver::MakeAnimation(const folly::dynamic& config)
  {
    ...
    std::chrono::milliseconds duration(static_cast<int>(m_velocity / -m_deceleration * 1000));
    animation.Duration(duration);

    const auto compositor = winrt::Window::Current().Compositor();
    animation.SetScalarParameter(s_velocityParameterName, static_cast<float>(m_velocity));
    animation.SetScalarParameter(s_decelerationParameterName, static_cast<float>(m_deceleration));
    animation.SetScalarParameter(s_durationName, static_cast<float>(m_velocity / -m_deceleration) * 1000);
    // Offset = (Velocity*time) + (0.5*Acceleration*Time^2)
    animation.InsertExpressionKeyFrame(1.0f, static_cast<winrt::hstring>(L"(") + s_durationName + L" * " + s_velocityParameterName + L") + (0.5 * " + s_decelerationParameterName + L" * " + s_durationName + L" * " + s_durationName + L")", compositor.CreateCubicBezierEasingFunction({ 0,1 }, { 0, 1 }));
