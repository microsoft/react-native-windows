...
facebook::react::JSECreator DevSupportManager::LoadJavaScriptInProxyMode(const facebook::react::DevSettings& settings)
{
  ...
  try
  {
    ...
    return [this, settings](std::shared_ptr<facebook::react::ExecutorDelegate> delegate, std::shared_ptr<facebook::react::MessageQueueThread> jsQueue)
    {
      auto websocketJSE = std::make_unique<WebSocketJSExecutor>(delegate, jsQueue);
      try
      {
        websocketJSE->ConnectAsync(facebook::react::DevServerHelper::get_WebsocketProxyUrl(settings.debugHost), settings.errorCallback).get();
      }
      catch (...)
      {
  ...
  {
    m_exceptionCaught = true;
    throw std::exception(facebook::react::UnicodeConversion::Utf16ToUtf8(e.message().c_str(), e.message().size()).c_str());
  }
}
...
std::future<winrt::Windows::Web::Http::HttpStatusCode> PollForLiveReload(const std::string& url)
{
  ...
  responseMessage = co_await httpClient.GetAsync(uri, winrt::Windows::Web::Http::HttpCompletionOption::ResponseHeadersRead);
  ...
  auto task = [refreshUrl, onChangeCallback = move(onChangeCallback), this](const std::atomic_bool& cancelled)
  {
    while (!cancelled)
    {
      ...
      catch (winrt::hresult_error const & e)
      {
        ...
        std::string errorMessage = "Live Reload Stopped:" + facebook::react::UnicodeConversion::Utf16ToUtf8(e.message().c_str(), e.message().size());
...
std::string DevSupportManager::GetJavaScriptFromServer(const std::string& debugHost, const std::string& jsBundleName, const std::string& platform)
{
  // Reset exception state since client is requesting new service
  m_exceptionCaught = false;
  ...
  {
    m_exceptionCaught = true;
    return "Error:" + facebook::react::UnicodeConversion::Utf16ToUtf8(e.message().c_str(), e.message().size());
  }
}
