...
std::vector<facebook::react::NativeModuleDescription> GetModules(
  ...
  std::shared_ptr<facebook::react::Theming> theme)
{
  // Modules
  ...
  modules.emplace_back(
    facebook::react::ThemingModule::name,
    [theme = std::move(theme)]() mutable { return std::make_unique<facebook::react::ThemingModule>(std::move(theme)); },
    std::make_shared<WorkerMessageQueueThread>());
...
void UwpReactInstance::Start(const std::shared_ptr<IReactInstance>& spThis, const ReactInstanceSettings& settings)
{
  ...
  std::shared_ptr<facebook::react::Theming> theme = std::make_shared<react::uwp::Theming>(spThis);
  ...
  m_initThread->runOnQueueSync(
    [this, spThis, deviceInfo, settings, i18nInfo = std::move(i18nInfo), appstate = std::move(appstate), theme = std::move(theme)]() mutable
  {
    ...
    std::vector<facebook::react::NativeModuleDescription> cxxModules = GetModules(m_uiManager, m_defaultNativeThread, deviceInfo, devSettings, std::move(i18nInfo), std::move(appstate), std::move(theme));
