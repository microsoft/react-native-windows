...
// It's better for performance to cache each unique ExpressionAnimation, as there's a one-time cost to parse
// the string down to tokens.  The Composition team designed ExpressionAnimations for reusability, each call to
// StartAnimation stamps out an animation with reference parameters fully resolved, so they can be reused.
