...
// Dead simple implementation with local filesystem storage using standard c++ fileio
// but with optional extension point with custom bufferStore.
class BasePreparedScriptStoreImpl : public facebook::jsi::PreparedScriptStore
{
public:
  std::shared_ptr<const facebook::jsi::Buffer> tryGetPreparedScript(const facebook::jsi::ScriptSignature& scriptSignature,
    const facebook::jsi::JSRuntimeSignature& runtimeSignature,
    const char* prepareTag) noexcept override;

  void persistPreparedScript(std::shared_ptr<const facebook::jsi::Buffer> preparedScript,
...
private:
  std::string getPreparedScriptFileName(const facebook::jsi::ScriptSignature& scriptMetadata, const facebook::jsi::JSRuntimeSignature& runtimeMetadata, const char* prepareTag);

  std::shared_ptr<BufferStore> bufferStore_;
};

// Dead simple script store implementation assuming that the script url is a local filesystam path
// and assuming the script version is the script size, but with extension point to provide custom version provider.
