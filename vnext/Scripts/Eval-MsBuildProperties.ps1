...
# given solution and project file. Simply pass in a semicolon delimited list
# of property names and you'll get key-value pairs.
#
# For example:
#
# .\Eval-MsBuildProperties.ps1 -SolutionFile "..\Microsoft.ReactNative.sln" -ProjectFile "..\Microsoft.ReactNative\Microsoft.ReactNative.vcxproj" -PropertyNames "ProjectGUID;ProjectName"
#
# will output:
#
...
function Get-MSBuildProperties {
    ...
    # Look for the specified PropertyNames and evaluate it
    $project.Properties | ForEach-Object -Process {
        if ($PropertyNames -contains $_.Name) {
            Write-Host "$($_.Name)=$($_.EvaluatedValue)"
        }
    }
}
...
Get-MSBuildProperties -MSBuildPath $MSBuildPath -SolutionPath $SolutionPath -ProjectPath $ProjectPath -PropertyNames $PropertyNames.Split(';') -ExtraMSBuildProps $ExtraMSBuildProps