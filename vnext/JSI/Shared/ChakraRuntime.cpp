...
facebook::jsi::Value ChakraRuntime::evaluateJavaScript(
  ...
  if (m_args->scriptStore && m_args->preparedScriptStore && !m_args->enableDebugging) {
    ScriptStore &scriptStore = *(m_args->scriptStore);
    PreparedScriptStore &preparedScriptStore = *(m_args->preparedScriptStore);

    ScriptSignature scriptSignature{sourceUrl, scriptStore.getScriptVersion(sourceUrl)};
    JSRuntimeSignature runtimeSignature{description(), Version()};
