...
const reactRepo = new MockFileRepository_1.MockReactFileRepository(reactFiles);
const ovrRepo = new MockFileRepository_1.MockWritableFileRepository(overrideFiles);
test('constructor - Duplicate Override Names', () => {
    expect(() => new Manifest_1.default([
        new Override_1.PlatformOverride({ file: 'aaa.windows.js' }),
        new Override_1.PlatformOverride({ file: 'aaa.windows.js' }),
    ])).toThrow();
});
test('constructor - Input Mutation Doesnt Affect Manifest', () => {
    const overrides = [
        new Override_1.PlatformOverride({ file: 'aaa.windows.js' }),
        new Override_1.PlatformOverride({ file: 'bbb.windows.js' }),
    ];
    const manifest = new Manifest_1.default(overrides);
...
test('validate - Empty Manifest', async () => {
    ...
    const errors = await manifest.validate(new MockFileRepository_1.MockWritableFileRepository([]), reactRepo);
    expect(errors).toEqual([]);
});
test('validate - All Overrides Listed', async () => {
    const manifest = new Manifest_1.default([
        new Override_1.PlatformOverride({ file: 'aaa\\aaa.windows.js' }),
        new Override_1.PlatformOverride({ file: 'aaa\\bbb.windows.js' }),
        new Override_1.PlatformOverride({ file: 'bbb\\ccc.win32.js' }),
    ]);
...
test('validate - Extra Override', async () => {
    const manifest = new Manifest_1.default([
        ...
        new Override_1.PlatformOverride({ file: 'aaa\\bbb.windows.js' }),
        new Override_1.PlatformOverride({ file: 'bbb\\ccc.win32.js' }),
        new Override_1.PlatformOverride({ file: 'bbb\\ddd.win32.js' }),
    ]);
    const errors = await manifest.validate(ovrRepo, reactRepo);
    expect(errors).toEqual([
...
test('validate - Multiple Valid Types', async () => {
    const manifest = new Manifest_1.default([
        new Override_1.PatchOverride({
            file: overrideFiles[0].filename,
            baseFile: reactFiles[0].filename,
            baseVersion: '0.61.5',
            baseHash: Hash_1.hashContent(reactFiles[0].content),
            issue: 4567,
        }),
        new Override_1.DerivedOverride({
            file: overrideFiles[1].filename,
            baseFile: reactFiles[1].filename,
            baseVersion: '0.60.6',
            baseHash: Hash_1.hashContent(reactFiles[1].content),
            issue: 4568,
        }),
        new Override_1.PlatformOverride({ file: overrideFiles[2].filename }),
    ]);
    const errors = await manifest.validate(ovrRepo, reactRepo);
    expect(errors).toEqual([]);
});
...
test('validate - Include Catches Errors', async () => {
    const manifest = new Manifest_1.default([new Override_1.PlatformOverride({ file: 'bbb\\ccc.win32.js' })], {
        includePatterns: ['aaa/aaa*', 'bbb/ccc*'],
    });
    const errors = await manifest.validate(ovrRepo, reactRepo);
    expect(errors).toEqual([
...
test('validate - Multiple Exclude Patterns', async () => {
    const manifest = new Manifest_1.default([new Override_1.PlatformOverride({ file: 'aaa\\aaa.windows.js' })], {
...
const sampleManifest = new Manifest_1.default([
    new Override_1.CopyOverride({
        file: overrideFiles[0].filename,
        baseFile: reactFiles[0].filename,
        baseVersion: '0.62.2',
        baseHash: Hash_1.hashContent(reactFiles[0].content),
...
test('addOverride - Simple', () => {
    const manifest = new Manifest_1.default([]);
    manifest.addOverride(new Override_1.PlatformOverride({ file: 'Never gonna run around and desert you' }));
    expect(manifest.hasOverride('Never gonna run around and desert you')).toBe(true);
});
test('addOverride - Duplicate', () => {
    const manifest = new Manifest_1.default([]);
    manifest.addOverride(new Override_1.PlatformOverride({ file: 'Never gonna make you cry' }));
    expect(() => manifest.addOverride(new Override_1.PlatformOverride({ file: 'Never gonna make you cry' }))).toThrow();
});
test('findOverride - None exists', () => {
    expect(sampleManifest.findOverride('Never gonna say goodbye')).toBeNull();
});
test('findOverride - Exists', () => {
    const override = new Override_1.PlatformOverride({
        file: 'Never gonna tell a lie and hurt you',
    });
    const manifest = new Manifest_1.default([override]);
    expect(manifest.findOverride('Never gonna tell a lie and hurt you')).toEqual(override);
});
test('markUpToDate - Simple', async () => {
    ...
    const overrideFactory = new OverrideFactory_1.OverrideFactoryImpl(reactRepo, ovrRepo);
    ...
    expect(newOverride.baseHash).toBe(Hash_1.hashContent(reactFiles[0].content));
});
test('Serialization Round-Trip', () => {
    ...
    expect(Manifest_1.default.fromSerialized(serializedManifest).serialize()).toEqual(serializedManifest);
});
test('String Exact Serialization Round-Trip', () => {
    ...
    const manifest = Manifest_1.default.fromSerialized(Serialized.parseManifest(manifestString));
