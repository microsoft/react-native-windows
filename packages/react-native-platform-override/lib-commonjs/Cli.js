...
async function upgrade(opts) {
    ...
    await spinnerGuard(spinner, async () => {
        const upgradeResults = await Api.upgradeOverrides(Object.assign(Object.assign({}, opts), { progressListener: (currentOverride, totalOverrides) => (spinner.text = `Merging overrides (${currentOverride}/${totalOverrides})`) }));
        spinner.succeed();
 ...
 */
async function printValidationErrors(validationErrors) {
    if (validationErrors.length === 0) {
        return;
    }
    const npmPackage = await PackageUtils_1.getNpmPackage();
