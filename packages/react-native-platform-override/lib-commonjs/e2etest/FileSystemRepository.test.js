...
test('listFiles - No Glob', async () => {
    ...
    await Resource_1.usingFiles(overrideFiles, async (repo) => {
...
test('listFiles - Multiple Globs', async () => {
    ...
    await Resource_1.usingFiles(overrideFiles, async (repo) => {
        expect((await repo.listFiles(['**/*.conflict', '**/*.addition'])).sort()).toEqual([
            '0.61.5/flowconfig.windows.conflict',
            '0.62.2/flowconfig.windows.addition',
        ]);
    });
...
test('writeFile - New File', async () => {
    await Resource_1.usingFiles([], async (repo) => {
...
test('stat - File', async () => {
    await Resource_1.usingFiles(['0.59.9/Icon-60@2x.conflict.png'], async (repo) => {
...
//# sourceMappingURL=FileSystemRepository.test.js.map