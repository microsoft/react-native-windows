...
exports.calcPackageDependencies = void 0;
 ...
 */
async function upgradeDependencies(newReactNativeVersion) {
    ...
    const localPackages = (await package_utils_1.enumerateRepoPackages()).map(pkg => (Object.assign(Object.assign({}, extractPackageDeps(pkg.json)), { outOfTreePlatform: OUT_OF_TREE_PLATFORMS.includes(pkg.json.name) })));
    const newDeps = calcPackageDependencies(newReactNativeVersion, reactNativeDiff, repoConfigDiff, localPackages);
    const writablePackages = await package_utils_1.enumerateRepoPackages();
    await Promise.all(newDeps.map(async (deps) => {
        const [writablePackage] = writablePackages.filter(p => p.json.name === deps.packageName);
        const oldJson = writablePackage.json;
        const newJson = Object.assign(oldJson, _.pick(deps, 'dependencies', 'peerDependencies', 'devDependencies'));
    ...
    await runCommand_1.default('yarn install');
}
 ...
 * across the monorepo
 */
async function upgradeReactNative(newReactNativeVersion) {
    const platformPackages = await package_utils_1.enumerateRepoPackages(async (pkg) => OUT_OF_TREE_PLATFORMS.includes(pkg.json.name));
    if (platformPackages.length === 0) {
        throw new Error('Expected to find a package for at least 1 out-of-tree platform');
    }
    const findRnOpts = { searchPath: platformPackages[0].path };
    const origJson = (await package_utils_1.findPackage('react-native', findRnOpts)).json;
    for (const pkg of await package_utils_1.enumerateRepoPackages()) {
        if (pkg.json.dependencies && pkg.json.dependencies['react-native']) {
            await pkg.mergeProps({
                dependencies: {
                    'react-native': bumpSemver(pkg.json.dependencies['react-native'], newReactNativeVersion),
                },
            });
        }
        if (pkg.json.devDependencies && pkg.json.devDependencies['react-native']) {
            await pkg.mergeProps({
                devDependencies: {
                    'react-native': bumpSemver(pkg.json.devDependencies['react-native'], newReactNativeVersion),
                },
            });
        }
    }
    await runCommand_1.default('yarn install');
    const newJson = (await package_utils_1.findPackage('react-native', findRnOpts)).json;
    return extractPackageDiff(origJson, newJson);
}
/**
 * Uses override tooling to pull in a new version of the repo-config package
 */
async function upgradeRepoConfig(newReactNativeVersion) {
    const origPackage = (await package_utils_1.findRepoPackage('@react-native/repo-config'));
    const upgradeResults = await react_native_platform_override_1.upgradeOverrides(path.join(origPackage.path, 'overrides.json'), {
        reactNativeVersion: newReactNativeVersion,
        allowConflicts: false,
    });
    if (!upgradeResults.every(result => result.filesWritten)) {
        throw new Error('Could not sync repo-config package due to conflicts. Please resolve manually');
    }
    const newPackage = (await package_utils_1.findRepoPackage('@react-native/repo-config'));
 ...
 */
function extractPackageDiff(origJson, newJson) {
    return {
 ...
 */
function extractPackageDeps(json) {
    return Object.assign(Object.assign(Object.assign({ packageName: json.name }, (json.dependencies && { dependencies: json.dependencies })), (json.peerDependencies && {
 ...
 * react-native.
 */
function calcPackageDependencies(newReactNativeVersion, reactNativePackageDiff, repoConfigPackageDiff, localPackages) {
    return localPackages.map(pkg => {
        const newPackage = _.cloneDeep(pkg);
        ...
        if (newPackage.dependencies && newPackage.dependencies['react-native']) {
            newPackage.dependencies['react-native'] = bumpSemver(newPackage.dependencies['react-native'], newReactNativeVersion);
        }
        ensureValidReactNativePeerDep(newPackage, newReactNativeVersion);
        ensureReactNativePeerDepsSatisfied(newPackage, reactNativePackageDiff.newPackage);
        syncDevDependencies(newPackage, repoConfigPackageDiff, reactNativePackageDiff);
...
exports.calcPackageDependencies = calcPackageDependencies;
/**
 ...
 */
function syncReactNativeDependencies(pkg, reactNativePackageDiff) {
    ...
    const extraDeps = _.difference(Object.keys(pkg.dependencies || {}), Object.keys(reactNativePackageDiff.oldPackage.dependencies || {}));
    const newDeps = Object.assign(Object.assign({}, _.pick(pkg.dependencies, extraDeps)), reactNativePackageDiff.newPackage.dependencies);
    ...
    const extraPeerDeps = _.difference(Object.keys(pkg.peerDependencies || {}), Object.keys(reactNativePackageDiff.oldPackage.peerDependencies || {}));
    const newPeerDeps = Object.assign(Object.assign({}, _.pick(pkg.peerDependencies, extraPeerDeps)), reactNativePackageDiff.newPackage.peerDependencies);
 ...
 */
function syncDevDependencies(pkg, repoConfigPackageDiff, reactNativePackageDiff) {
    ...
    const newRNDevDevDeps = reactNativePackageDiff.newPackage.devDependencies || {};
    ...
    for (const [dependency, version] of devDependencies) {
        if (pkg.outOfTreePlatform && newRNDevDevDeps.hasOwnProperty(dependency)) {
            if (semver.gt(semver.minVersion(newRNDevDevDeps[dependency]), semver.minVersion(version))) {
                pkg.devDependencies[dependency] = newRNDevDevDeps[dependency];
            }
        }
        else if (newRepoConfigDeps.hasOwnProperty(dependency)) {
            if (semver.gt(semver.minVersion(newRepoConfigDeps[dependency]), semver.minVersion(version))) {
 ...
 */
function ensureValidReactNativePeerDep(pkg, newReactNativeVersion) {
    ...
    pkg.peerDependencies['react-native'] === '*' ||
        semver.satisfies(newReactNativeVersion, pkg.peerDependencies['react-native'])) {
        return;
    }
    // Use a heuristic to bump otherwise
    pkg.peerDependencies['react-native'] = bumpSemver(pkg.peerDependencies['react-native'], newReactNativeVersion);
}
/**
 * Ensure that a package fulfills peer depenedncies for react-native if relying on it
 */
function ensureReactNativePeerDepsSatisfied(pkg, newReactNativePkg) {
 ...
 */
function bumpSemver(origVersion, newVersion) {
