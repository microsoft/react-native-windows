...
function queryNoOpenFiles() {
    const opened = child_process_1.execSync('git status -s').toString();
    if (opened) {
...
function spawnClangFormat(args, done, stdio) {
    ...
    let nativeBinary;
    try {
        nativeBinary = clang_format_1.getNativeBinary();
    }
    catch (e) {
    ...
    files = files.filter(file => includeEndsWith.some(_ => file.endsWith(_)) &&
    ...
    async.series(chunks.map(function (chunk) {
        return function (callback) {
            const clangFormatProcess = child_process_1.spawn(nativeBinary, args.concat(chunk), {
                stdio: stdio,
            });
