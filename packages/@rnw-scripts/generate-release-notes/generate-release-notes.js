...
if (!GITHUB_TOKEN) {
  ...
  if (!REPO) {
    console.error('GITHUB_REPOSITORY is not set. Please set it (e.g. microsoft/react-native-windows).');
    process.exit(1);
  }
...
function isBotCommit(commit) {
  ...
  if (botIndicators.some(bot => authorLogin.includes(bot) || commitAuthorName.includes(bot))) return true;
  if (['bump', 'applying package updates', 'no_ci', 'no ci'].some(k => msg.includes(k))) return true;
  return false;
}
...
function categorizeCommits(commits) {
  ...
  for (const c of commits) {
    ...
    const matched = Object.keys(keywords).filter(k => keywords[k].some(word => lowerMsg.includes(word)));
    const category =
      matched.includes("Breaking Changes") ? "Breaking Changes" :
      matched.includes("New Features") ? "New Features" :
      matched.includes("Reliability") ? "Reliability" :
      matched.includes("New Architecture-specific changes") ? "New Architecture-specific changes" :
      "Other";
...
function generateReleaseNotes(commits, categories) {
  ...
  lines.push(`We're excited to release React Native Windows ${RELEASE_TAG} targeting React Native ${RELEASE_TAG}!`);
  lines.push(`This release includes the commits to React Native Windows from ${start} - ${end}.\n`);
  lines.push("## How to upgrade");
  lines.push("You can view the changes made to the default new React Native Windows applications for C++ and C# using React Native Upgrade Helper. See this [document](https://microsoft.github.io/react-native-windows/docs/upgrade-app) for more details.\n");
