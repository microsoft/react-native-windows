...
(async () => {
    ...
    if (argv.release === 'preview') {
        ...
        const [rootPkg] = await enumeratePackages(pkgJson => pkgJson.name === 'react-native-windows-repo');
...
async function writeAdoVariables(vars) {
    const adoPath = path.join(await find_repo_root_1.default(), '.ado');
    const releaseVariablesPath = path.join(adoPath, 'variables', 'release.yml');
    const releaseVariablesContent = '# Warning: This file is autogenerated by @rnw-scripts/promote-release\n' +
 ...
 * @param version major + minor version
 */
async function updateBeachballConfig(packagePath, release, version) {
    switch (release) {
        case 'preview':
 ...
 */
async function updatePackage(packagePath, props) {
    const packageJson = await jsonOfPackage(packagePath);
    _.merge(packageJson, props);
    await fs.promises.writeFile(packagePath, JSON.stringify(packageJson, null /*replacer*/, 2 /*space*/) + '\n');
}
/**
 * Returns the JSON content of a package
 */
async function jsonOfPackage(packagePath) {
 ...
 */
async function nameOfPackage(packagePath) {
 ...
 */
async function enumeratePackagesToPromote() {
 ...
 * @returns absolute paths to package.json
 */
async function enumeratePackages(pred = () => true) {
    const repoRoot = await find_repo_root_1.default();
    const allPackages = await glob('**/package.json', {
        cwd: repoRoot,
        ignore: '**/node_modules/**',
        absolute: true,
    });
...
async function updatePackageVersions(version) {
    ...
    const promotedPackages = await Promise.all(packagesToPromote.map(nameOfPackage));
...
async function createChangeFiles(changeType, message) {
    const repoRoot = await find_repo_root_1.default();
    child_process.execSync(`npx beachball change --type ${changeType} --message "${message}"`, { cwd: repoRoot, stdio: 'ignore' });
}
//# sourceMappingURL=promoteRelease.js.map