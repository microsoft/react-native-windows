...
> {
    ...
    return (
      <View style={styles.testRow}>
        <View style={styles.controlsView}>
          {this._renderOption('hasStyle', this.state.hasStyle, value =>
            this.setState({hasStyle: value}),
          )}
          {this._renderOption('hasBorder', this.state.hasBorder, value =>
            this.setState({hasBorder: value}),
          )}
          {this._renderOption('hasRadius', this.state.hasRadius, value =>
            this.setState({hasRadius: value}),
          )}
          {this._renderOption('largeBorder', this.state.largeBorder, value =>
            this.setState({largeBorder: value}),
          )}
          {this._renderOption('largePadding', this.state.largePadding, value =>
            this.setState({largePadding: value}),
          )}
          {this._renderOption('alignCenter', this.state.alignCenter, value =>
            this.setState({alignCenter: value}),
          )}
        </View>
        <View style={styles.styledView}>
          <View {...viewProps}>
            <Text
              style={[
                styles.child,
                {alignSelf: this.state.alignCenter ? 'center' : 'flex-start'},
              ]}>
              The Text
            </Text>
          </View>
      ...
      <View style={styles.testRow}>
        <View style={styles.controlsView}>
          {this._renderOption(
            ...
            value => this.setState({focusable: value}),
          )}
          {this._renderOption(
            'enablePointerEvents',
            this.state.enablePointerEvents,
            value => this.setState({enablePointerEvents: value}),
          )}
        </View>
        <View style={styles.hoverView} {...viewProps}>
          <Text>
            {this.state.isMouseOver ? 'Mouse is over' : 'Mouse is not over'}
          </Text>
          <Text>
            {this.state.lastPressedKey !== undefined
              ? `lastKey = '${this.state.lastPressedKey}'`
              : ''}
          </Text>
        </View>
      </View>
  ...
  private _onMouseEnter = (_e: any) => {
    this.setState({isMouseOver: true});
  };

  private _onMouseLeave = (_e: any) => {
