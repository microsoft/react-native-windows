  ...
  return {
      facebook::xplat::module::CxxModule::Method(
          ...
          [this](folly::dynamic args, Callback cb) {
            folly::dynamic d = folly::dynamic::object("Message", "Hello World!")(
                "Data",
                folly::dynamic::array(
...
void HostingPane::UpdateUI() {
  ...
  x_LoadButton->IsEnabled = !x_JavaScriptFilename->SelectedItem->ToString()->IsEmpty() &&
...
static winrt::Windows::Storage::ApplicationDataContainer GetJsSettings() {
  ...
  auto container =
      localSettings.CreateContainer(c_containerName, winrt::Windows::Storage::ApplicationDataCreateDisposition::Always);
...
void HostingPane::LoadFilenameSettings() {
  ...
  if (container.Values().HasKey(c_filenameSetting)) {
    filename =
        container.Values().Lookup(c_filenameSetting).as<winrt::Windows::Foundation::IPropertyValue>().GetString();
  } else {
    // Default
    filename = c_filenameSettingDefault;
  }

  String ^ jsFileName = reinterpret_cast<Platform::String ^>(winrt::get_abi(filename));
  ...
  String ^ reactAppName = reinterpret_cast<Platform::String ^>(winrt::get_abi(appname));
...
void HostingPane::StoreFilenameSettings() {
  ...
  const wchar_t *currentFilename = x_JavaScriptFilename->SelectedItem->ToString()->Data();
  const wchar_t *currentAppname = x_ReactAppName->SelectedItem->ToString()->Data();

  container.Values().Insert(
      c_filenameSetting, winrt::Windows::Foundation::PropertyValue::CreateString(currentFilename));
  container.Values().Insert(c_appnameSetting, winrt::Windows::Foundation::PropertyValue::CreateString(currentAppname));
...
void HostingPane::LoadKnownApps() {
  ...
  std::wstring jsFileName = x_JavaScriptFilename->SelectedItem->ToString()->Data();
