...
try {
  ...
  // Fallback to basic config if there are ESM loader issues
  console.warn('Using fallback Metro config due to ESM loader issue:', error.message);
  const {getDefaultConfig} = require('@react-native/metro-config');
  config = getDefaultConfig(__dirname);
  
  // Enhanced resolver configuration for fallback
  ...
  path.resolve(__dirname, '../../node_modules'),
  ...(originalResolver.nodeModulesPaths || []),
  ],
  platforms: ['windows', 'native', 'android', 'ios'],
  };
}
