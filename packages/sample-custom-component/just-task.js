...
task('codegenNativeComponents', () => {
  ...
  matches.forEach(matchedPath => {
    ...
    fs.writeFileSync(path.resolve(process.cwd(), 'lib-commonjs', relativeOutputPath), res?.code);
  });
});


task('buildWithCodegetNativeComponents', series('build', 'codegenNativeComponents'));
