...
public:
	...
	std::string GenerateScreenshot(HWND hwnd, RECT* prect = nullptr) noexcept
	{
		...
		// We're screenshotting the screen here. Ideally we'd screenshot the window (so it doesn't matter which window is on top)
		// but that seems to work for only the first screenshot...
        ...
        m_hbm = std::move(std::unique_ptr<std::remove_pointer<HBITMAP>::type, BitmapDeleter>(CreateCompatibleBitmap(hdcWnd.get(), rcClient.right - rcClient.left, rcClient.bottom - rcClient.top)));
		...
		if (!BitBlt(hdcMem.get(), 0, 0, rcClient.right - rcClient.left, rcClient.bottom - rcClient.top, hdcWnd.get(), rcClient.left, rcClient.top, SRCCOPY))
