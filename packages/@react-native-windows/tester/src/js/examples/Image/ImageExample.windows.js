...
// Base URL the remote Image examples load their assets from. The Windows E2E
// test app overrides it (see packages/e2e-test-app-fabric/imageFixtureConfig.js)
// with a loopback fixture server serving the same files, so the tests that only
// render once a download has completed do not depend on public network access.
const REMOTE_IMAGE_BASE_URL =
  'https://raw.githubusercontent.com/microsoft/react-native-windows/main/packages/@react-native/tester/js/assets/';
const IMAGE_BASE_URL: string =
  (global: $FlowFixMe).__RNW_E2E_IMAGE_BASE_URL__ ?? REMOTE_IMAGE_BASE_URL;

const IMAGE1 = `${IMAGE_BASE_URL}flux@3x.png`;
const IMAGE2 = `${IMAGE_BASE_URL}flowers.png`;
