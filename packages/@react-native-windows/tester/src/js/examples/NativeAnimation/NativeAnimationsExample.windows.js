    ...
    return (
      <View>
        ...
        <RNTesterSettingSwitchRow
          ...
          onEnable={() => {
            /*#6210*/ require('react-native/Libraries/Interaction/JSEventLoopWatchdog').install(
              {
                thresholdMS: 25,
              },
            );
            this.setState({busyTime: '<none>'});
            /*#6210*/ require('react-native/Libraries/Interaction/JSEventLoopWatchdog').addHandler(
              {
                onStall: ({busyTime}) =>
                  this.setState(state => ({
                    busyTime,
