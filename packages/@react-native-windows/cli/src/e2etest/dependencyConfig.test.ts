...
test.each(projects)(
  ...
  async (name, setup) => {
    ...
    expect(await dependencyConfigWindows(folder, userConfig)).toBe(
      expectedConfig,
    );
  },
);
...
test.each(projects)(
  ...
  async (name, setup) => {
    ...
    if (name === 'BlankLib') {
      expect(
        await dependencyConfigWindows(folder, userConfig),
      ).toMatchSnapshot();
    } else {
      expect(await dependencyConfigWindows(folder, userConfig)).toMatchSnapshot(
        {
