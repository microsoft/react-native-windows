...
ChakraValue ChakraObject::callAsFunction(JsValueRef thisObj, int nArgs, const JsValueRef args[]) const
{
  ...
  JsValueRef value;
  auto result = JsCallFunction(m_obj, const_cast<JsValueRef *>(argsWithThis), static_cast<unsigned short>(nArgs + 1), &value);
  delete[] argsWithThis;
  if (result != JsNoError)
  {
    bool hasException = false;
...
ChakraObject ChakraObject::callAsConstructor(std::initializer_list<JsValueRef> args) const
{
  ...
  JsValueRef value;
  auto result = JsConstructObject(m_obj, const_cast<JsValueRef *>(args.begin()), static_cast<unsigned short>(args.size()), &value);
