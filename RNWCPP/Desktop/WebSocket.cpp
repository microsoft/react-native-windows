...
template<typename Protocol, typename Socket, typename Resolver>
void BaseWebSocket<Protocol, Socket, Resolver>::Read()
{
...
template<typename Protocol, typename Socket, typename Resolver>
void BaseWebSocket<Protocol, Socket, Resolver>::EnqueueWrite(const std::string& message, bool binary)
{
...
template<typename Protocol, typename Socket, typename Resolver>
void BaseWebSocket<Protocol, Socket, Resolver>::SetOnClose(std::function<void(CloseCode, const std::string&)>&& handler)
{
...
/*static*/ std::unique_ptr<IWebSocket> IWebSocket::Make(const string& urlString)
{
  Url url(urlString);
  ...
  if (url.scheme == "ws")
  {
    ...
    return std::unique_ptr<IWebSocket>(new WebSocket<tcp>(std::move(url)));
  }
  else if (url.scheme == "wss")
  {
    ...
    return std::unique_ptr<IWebSocket>(new SecureWebSocket<tcp, ssl::stream<tcp::socket>>(std::move(url)));
  }
  else
