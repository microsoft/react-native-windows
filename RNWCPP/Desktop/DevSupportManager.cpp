...
JSECreator DevSupportManager::LoadJavaScriptInProxyMode(const DevSettings& settings)
{
  return [settings](shared_ptr<ExecutorDelegate> delegate, shared_ptr<MessageQueueThread> jsQueue)
  {
    ...
    websocketJSE->ConnectAsync(DevServerHelper::get_WebsocketProxyUrl(settings.debugHost), settings.errorCallback).wait();
    return websocketJSE;
  };
}

string DevSupportManager::GetJavaScriptFromServer(const string& debugHost, const string& jsBundleName, const string& platform)
{
  auto bundleUrl = DevServerHelper::get_BundleUrl(debugHost, jsBundleName, platform /*platform*/, "true" /*dev*/, "false" /*hot*/);
