...
{
  // TODO: This is very inefficient, but doing this with a heap is inherently hard. If performance is not good
...
void Timing::OnTimerRaised() noexcept
{
  if (auto inst = m_wkInstance.lock())
  {
    if (auto nativeThread = m_nativeThread.lock())
    {
      ...
      // Capture weak_ptr "this" because callback will be executed on native thread even if "this" is destroyed.
      nativeThread->runOnQueue(
        [weakThis = std::weak_ptr<Timing>(shared_from_this())](){
          ...
          while (!strongThis->m_timerQueue.IsEmpty() && now_ms > strongThis->m_timerQueue.Front().DueTime - 10ms)
          {
            ...
            // 'next.Period' being greater than 10ms is intended to prevent infinite loops
            if (next.Repeat)
...
void Timing::createTimer(std::weak_ptr<facebook::react::Instance> instance, uint64_t id, double duration, double jsSchedulingTime, bool repeat) noexcept
{
...
void Timing::TimersChanged() noexcept
{
  ...
  // If current front timer's due time is earlier than current ThreadpoolTimer's,
  // we need to reset the ThreadpoolTimer to current front timer
...
{
  // Here we assume if kernel timer is going to fire within 2 frames (about 33ms), we return true
  // I am not sure the 2 frames assumption is good enough. We may need adjustment after performance analysis.
  auto now = std::chrono::system_clock::now();
  auto now_ms = std::chrono::time_point_cast<std::chrono::milliseconds>(now);
  if(m_dueTime - now_ms <= 33ms) return true;
  return false;
}
...
{
  if(m_timerQueue.IsEmpty()) return;
...
std::vector<module::CxxModule::Method> TimingModule::getMethods() noexcept
{
  return
  {
    Method("createTimer", [this](dynamic args) // int64_t id, int64_t duration, double jsSchedulingTime, bool repeat
      {
        m_timing->createTimer(getInstance(), jsArgAsInt(args, 0), jsArgAsDouble(args, 1), jsArgAsDouble(args, 2), jsArgAsBool(args, 3));
      }),
    Method("deleteTimer", [this](dynamic args) // int64_t code, const std::string& reason, int64_t id
      {
        m_timing->deleteTimer(jsArgAsInt(args, 0));
      }),
    Method("setSendIdleEvents", [this](dynamic args) // const std::string& message, int64_t id
      {
