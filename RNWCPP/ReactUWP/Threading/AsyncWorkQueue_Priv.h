...
class AsyncWorkQueue :
  public IAsyncWorkQueue
{
private:
  /// <summary>
  /// Represents an individual work item. For each work item queued, AsyncWorkQueue
  /// creates an AsyncWorkItem to store the information about the request.
  /// </summary>
...
private:
  ...
  ///   [in] A TP_CALLBACK_INSTANCE structure that defines the callback instance.
  ///   Applications do not modify the members of this structure.
  /// </param>
  /// <param name="pContext">
  ///   [in] The application-defined data, supplied as second parameter to CreateThreadpoolWork.
  /// </param>
  /// <param name="pWork">
  ///   [in] A TP_WORK structure that defines the work object that generated the callback.
  /// </param>
  ...
  /// <summary>Callback for threadpool. Delegated to by s_WorkCallBack().</summary>
  VOID _AsyncCallback();
...
private:
  ...
  // m_lock protects m_workItems, m_pProcessingWorkItem, m_workSubmitted, and m_permanentlyShutDown
  ...
  //tlx::auto_xxx<PTP_WORK, VOID(__stdcall*)(PTP_WORK), CloseThreadpoolWork, NULL> m_tpWork;
