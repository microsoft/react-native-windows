...
//{
  ...
  // AsyncWorkQueue object. If the clients fail to call to ShutDown() an outstanding
  // worker thread may access the freed AsyncWorkQueue object and cause an access violation.
  //
  // We can't call ShutDown for the clients, as that would cause a dead lock. If we did
  // and the client doesn't call ShutDown() before it releases the AsyncWorkQueue,
  // we would get into the situation that the IAsyncCallback holds the last reference
  // to the client object which holds the last reference back to the AsyncWorkQueue.
  // When the IAsyncCallback Release() is processed, AsyncWorkQueue is going
  // to be destroyed on the  worker thread. Thus calling ShutDown here causes
  // WaitForThreadpoolWorkCallbacks on a worker thread, that's the dead lock of "wait
...
COM_DECLSPEC_NOTHROW STDMETHODIMP AsyncWorkQueue::QueryInterface(
  REFIID riid,
  ...
  _In_ PTP_WORK pWork)
{
  ...
  // We must handle a subtle case when the IAsyncCallback implementation holds a reference to a client object
  // which holds a reference back to the AsyncWorkQueue:
  //
  //   IAsyncCallback object --> client object --> AsyncWorkQueue
  //
  // The IAsyncCallback object will be released after processed. At that time if it holds the last reference
  // to client object and client object holds the last reference to AsyncWorkQueue, AsyncWorkQueue will be
  // destroyed. Thus when AsyncWorkQueue::_AsyncCallback() is processing the next work item (in the next iteration
  // in the for loop) and trying to get the lock (m_lock), it is accessing a freed memory.
  //
  // To resolve this problem, we must increase the ref count to AsyncWorkQueue to prevent it being destroyed
  // inside the AsyncWorkQueue::_AsyncCallback(), but we cannot simply call AddRef() before and Release() after
  // AsyncWorkQueue::_AsyncCallback(). The trick here is that, the implementation of our IUnknown::Release()
  // decreases the ref count *before* calling destructor. So when one thread is in the destructor the ref count
  // is already zero,  AddRef() and Release() cause the ref count bump up to 1 and drop to 0 and eventually cause
  // destructor be called again.
  //
  // We solve this by doing the following:
  // 1. AddRef() returning 1 means the AsyncWorkQueue is being destroyed. When detected we stop processing
  //    IAsyncCallback objects.
  // 2. WaitForThreadpoolWorkCallbacks() in the destructor makes sure the memory won�t be freed until all
...
VOID AsyncWorkQueue::_AsyncCallback()
{
  ...
  for (;;)
  {
    // Attempt to pop one work item off the front of the queue. If there isn't one then exit.
    // The lock must be engaged for this entire process to ensure protection of m_workItems
    // and m_pProcessingWorkItem in tandem.
    {
...
VOID AsyncWorkQueue::_CancelAllAndWait(_In_ BOOL permanentlyShutDown)
{
  ...
  if (m_tpWork.get() != nullptr)
  {
    // The WaitForThreadpoolWorkCallbacks function blocks the calling thread until all
    // outstanding callback functions for a work object have completed execution.
    // Passing FALSE for the 2nd parameter allows the callbacks that have been queued to the
    // thread pool to be dispatched to worker thread for execution.  However since
    // _CancelAllAsync has emptied m_workItems, no workItem's ProcessWorkItem will execute,
    // but we will reset all of our state variables
    ::WaitForThreadpoolWorkCallbacks(m_tpWork.get(), FALSE);
  }
}
