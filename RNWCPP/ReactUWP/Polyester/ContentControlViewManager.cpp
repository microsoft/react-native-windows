...
public:
  ...
  bool IsExternalLayoutDirty() const override { return m_paddingDirty; }
...
void ContentControlShadowNode::createView()
{
  ...
  if (obj != nullptr)
  {
    ...
    obj.RegisterPropertyChangedCallback(winrt::Windows::UI::Xaml::Controls::Control::PaddingProperty(), [this](winrt::Windows::UI::Xaml::DependencyObject const& sender, winrt::Windows::UI::Xaml::DependencyProperty const& dp)
    {
      m_paddingDirty = true;
    });
  }
...
void ContentControlViewManager::AddView(XamlView parent, XamlView child, int64_t index)
{
  ...
  if (index != 0)
  {
    // ASSERT: Currently considering any index other than 0 as ignorable since ContentControl can only have a single child.
    assert(false);
    return;
  }
