...
Timing::Timing(TimingModule* parent)
  : m_parent(parent)
{
}
...
void Timing::OnRendering(const winrt::IInspectable&, const winrt::IInspectable& args)
{
  ...
  if (!readyTimers.empty())
  {
    ...
    {
      // Package list of Timer Ids to fire in a dynamic array to pass as parameter
...
TimingModule::TimingModule()
  : m_timing(std::make_shared<Timing>(this))
{
}
...
auto TimingModule::getMethods() -> std::vector<Method>
{
  ...
  return
  {
    Method("createTimer", [timing](dynamic args) // int64_t id, double duration, double jsSchedulingTime, bool repeat
      {
        timing->createTimer(jsArgAsInt(args, 0), jsArgAsDouble(args, 1), jsArgAsDouble(args, 2), jsArgAsBool(args, 3));
      }),
    Method("deleteTimer", [timing](dynamic args) // int64_t code, const std::string& reason, int64_t id
      {
        timing->deleteTimer(jsArgAsInt(args, 0));
      }),
    Method("setSendIdleEvents", [timing](dynamic args) // const std::string& message, int64_t id
      {
...
std::unique_ptr<facebook::xplat::module::CxxModule> CreateTimingModule(const std::shared_ptr<facebook::react::MessageQueueThread>&) noexcept
{
